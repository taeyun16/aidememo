...
| 훅 | 매처 | 역할 | 비용 |
|---|---|---|---|
| `aidememo-session-start.py` | `SessionStart`, `*` | pinned + recent + overview를 새 세션 시작 시 inject | 로컬 only (~50 ms) |
| `aidememo-post-tool.py` | `PostToolUse`, `Edit\|Write\|MultiEdit` | 편집한 파일 관련 aidememo 사실 surface | 로컬 only (~30 ms) |
| `aidememo-extract-facts.py` | `UserPromptSubmit`, `*` (≥ 200자) | user prompt에서 fact 후보 추출 (preview only) | 기본 heuristic (~5 ms); `AIDEMEMO_EXTRACT_LLM=1`이면 OpenAI 호출 (~$0.0001/prompt) |

세 훅 모두:
- soft-fail (aidememo 바이너리 부재 / store 없음 등에 세션 안 막음)
- `additionalContext` 필드로 inject (block 안 함)
- 환경변수: `AIDEMEMO_BIN`, `AIDEMEMO_STORE`, `AIDEMEMO_PROJECT`로 store 위치 지정
...
cp ~/dev/aidememo/aidememo-skill/hooks/aidememo-*.py ~/.claude/hooks/
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.claude/hooks/aidememo-session-start.py",
            "statusMessage": "aidememo: loading wiki context…"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.claude/hooks/aidememo-post-tool.py"
          }
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$HOME/.claude/hooks/aidememo-extract-facts.py"
          }
        ]
      }
    ]
...
LLM 추출 활성:
```bash
export AIDEMEMO_EXTRACT_LLM=1         # extract.provider 설정 필요
```
...
echo '{"tool_name":"Edit","tool_input":{"file_path":"crates/aidememo-core/src/lib.rs"}}' \
...
echo '{"prompt":"We decided to migrate Postgres replicas to read-only mode after the 2026-04-12 outage. Auth flow now uses Keycloak with JWT-Service issuing 15min tokens."}' \
  | python3 ~/.claude/hooks/aidememo-extract-facts.py | jq .
```
...
- **LLM 호출 opt-in only**. `AIDEMEMO_EXTRACT_LLM=1` 명시할 때만.
- **타임아웃**: 4-8초 hard cap. 느린 aidememo 호출이 세션을 막지 않음.
- **silent on failure**. aidememo 부재 / store 없음 / 에러 → 세션 정상 진행.

## 한계

- `Stop` 훅 (session summary 자동 생성) 은 **포함하지 않음**.
  세션 요약을 자동으로 fact로 저장하면 노이즈 위험. 사용자가 명시적
  `aidememo fact add` 또는 `aidememo consolidate`로 정제하는 게 안전.
- `aidememo-extract-facts.py`는 `additionalContext`로 후보를 surface 하지만
  Claude가 자동 적용하지 않음. agent가 `aidememo_fact_add` 명시 호출 필요.
