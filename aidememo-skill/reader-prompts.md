...
모두 reader 단계의 prompt 엔지니어링 — aidememo가 retrieval로 evidence를
가져와도 reader가 어떻게 처리하느냐에 따라 답이 갈림. 이 문서는
같은 패턴을 aidememo + 임의의 reader (gpt-4o / gpt-4.1 / MiniMax / Claude /
Gemini) 사용 시 적용할 수 있도록 정리.

> 모두 aidememo가 직접 적용하는 게 아닌, **agent가 reader를 호출할 때 추가할
...
aidememo는 search response에 `created_at` / `observed_at` / `age_days` /
`freshness_warning` 필드를 슬림 페이로드로 전달 (`crates/aidememo-cli/src/cmd/mcp_tools.rs::slim_fact_record`).
...
correct answer. AideMemo's `current_only` filter and `superseded_by`
...
aidememo는 default `current_only=true`로 superseded fact 제외. reader가
...
aidememo + 이 변환된 query → hybrid_search 정확도 증가.
...
reader prompt에 **aidememo_context의 personalisation 섹션 활용** 명시:
...
aidememo `aidememo_context` 응답에 `personalisation` 섹션 자동 surface (Tier A3).
...
3. OMEGA의 reader prompt 효과는 다른 baseline 대비 (이미 강한 aidememo
...
영역 외 작업 필요. `aidememo ingest --llm` / hook auto-apply / async LLM
extract 등이 진짜 ROI.
...
- **aidememo 슬림 페이로드는 `age_days` / `freshness_warning` 노출**하지만,
  `created_at` / `observed_at` 절대 시각은 노출 안 함 (토큰 절약).
  Temporal prompting 적용 시 fact_get으로 추가 fetch 필요할 수도.
- **personalisation 섹션은 aidememo_context 호출에서만 surface** —
  aidememo_search / aidememo_query는 자동 포함 X. agent가 aidememo_context를
  top-of-turn entry point로 일관 사용해야 효과 발휘.
- **Query augmentation은 사용자 자체 LLM 호출** — aidememo 영역 밖. agent
  가 aidememo_search 호출 전에 자체 rewrite 단계 추가 필요.
...
- [`docs/MEASUREMENTS.md`](../docs/MEASUREMENTS.md) — aidememo vs OMEGA 격차 분해
- [`docs/MEASUREMENTS.md`](../docs/MEASUREMENTS.md) — aidememo 측정 결과 + LongMemEval-S 데이터셋 noise 분석
