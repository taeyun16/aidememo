#!/usr/bin/env python3
"""SessionStart hook for Claude Code — surfaces the wg wiki's pinned
...
local `wg overview` + `wg recent` shell-out, no API calls. Output is
injected as `additionalContext` so the agent sees it before the user's
first prompt.

Install:
  cp wg-session-start.py ~/.claude/hooks/wg-session-start.py
  chmod +x ~/.claude/hooks/wg-session-start.py
...
def run_wg(*args: str, timeout: int = 5) -> str | None:
    """Invoke `wg` with optional --store / --project from env, capture
    stdout. Returns None on any failure so the hook stays soft-fail —
    a dead wg binary should never block the session."""
    cmd = [os.environ.get("WG_BIN", "wg")]
    if store := os.environ.get("WG_STORE"):
        cmd += ["--store", store]
    elif project := os.environ.get("WG_PROJECT"):
        cmd += ["--project", project]
    cmd += list(args)
    try:
...
def main() -> int:
    ...
    overview = run_wg("overview", "-n", "5")
    recent = run_wg("recent", "-n", "10", "--last", "7d")
    pinned = run_wg("fact", "pinned", "--limit", "10")
    ...
    body = (
        "## wg wiki context (auto-loaded by hook)\n\n"
        + "\n\n".join(sections)
        + "\n\nUse `wg_query <topic>` / `wg_search <q>` for follow-up retrieval."
    )
