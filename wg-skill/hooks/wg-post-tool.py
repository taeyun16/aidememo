...
wg facts related to the file just edited so the agent sees the
relevant decisions / patterns / conventions before responding.

Cost: one local `wg query` per matching tool call. No API.

Install:
  cp wg-post-tool.py ~/.claude/hooks/wg-post-tool.py
  chmod +x ~/.claude/hooks/wg-post-tool.py
...
def run_wg(*args: str, timeout: int = 4) -> str | None:
...
def candidate_topics(file_path: str) -> list[str]:
    ...
    'crates/wg-core/src/lib.rs' →
    ['lib', 'wg-core', 'wg-core/src/lib.rs']
    so wg can match against entity names (`wg-core`, `lib.rs`) AND
    ...
    # for repo-shaped wikis (crates/wg-core/src/lib.rs → "wg-core").
    if len(parts) >= 3:
        parent = parts[-3]
...
def main() -> int:
    ...
    for topic in candidate_topics(file_path):
        out = run_wg("query", topic, "-l", "3", "--recent-limit", "3")
        if out and "no results" not in out.lower() and out.strip():
            body = (
                f"## wg facts related to `{file_path}` (via topic '{topic}')\n\n"
                f"{out}\n\n"
                "If anything contradicts the change you just made, consider "
                "`wg_fact_supersede` to retire the stale fact."
            )
