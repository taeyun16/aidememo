...
[`docs/`](docs/)의 영문 기준 Markdown을 렌더링합니다. 영어는 `/aidememo/`,
한국어는 `/aidememo/ko/`에서 제공됩니다. 온보딩과 워크플로 문서는
...
SDK는 `PATH`에 있는 `aidememo` CLI로 폴백합니다. 선택형 네이티브 바인딩
빠른 경로는 `aidememo-python` PyPI 릴리스 후
`python -m pip install "aidememo-agent-sdk[binding]"`으로 제공됩니다.
