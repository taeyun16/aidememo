...
allowed-tools: Bash(./target/debug/aidememo query:*), Bash(aidememo query:*)
---

Build a context dossier for "$ARGUMENTS" before answering or coding.

Run `aidememo --json query "$ARGUMENTS" --limit 8 --depth 2 --recent-limit 8`.
...
`/aidememo-add-fact` to record what we discover during the work.
