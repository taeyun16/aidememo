"""Thin wrapper for agent code that talks to wg through one API.

Callers do not need to care whether the in-process binding
(``wg-python``) or the subprocess CLI is available.
...
class WgClient:
    ...
    def context(
        ...
        payload: dict[str, Any] = {
            ...
            "recent": recent,
            "backend_note": (
                "wg-agent-sdk is using wg-python; pinned facts are not exposed by the binding yet, "
    ...
    def doctor(self) -> dict:
        ...
        return {
            ...
            "sharing": {
                ...
                "hint": "wg-python owns the redb handle in this process; use daemon/MCP for high-concurrency shared writes.",
            },
        }
    ...
    def workflow_start(
        ...
        the redb handle does not shell out to a second `wg` process and fight
    ...
    def _mcp_tool(self, name: str, arguments: dict[str, Any]) -> Any:
        init = {
            ...
            "id": 0,
            "method": "initialize",
            "params": {"clientInfo": {"name": "wg-agent-sdk", "version": "0.1.0"}},
        }
        call = {
            "jsonrpc": "2.0",
            "id": 1,
