#!/usr/bin/env python3
"""Synthetic dataset generator for the aidememo vs beads comparison.
...
Two outputs (same N, same content order, fixed seed):
  - corpus_aidememo.jsonl     {"entity": str, "content": str}
  - corpus_beads.jsonl  {"title": str, "description": str,
                         "issue_type": "task", "priority": 2}

Run:
  python3 bench/beads-vs-aidememo/gen.py --n 1000 --seed 42 --out bench/beads-vs-aidememo/data
"""
...
# 5 entity buckets so that aidememo's entity-based search has something to
...
def main() -> int:
    ...
    ap.add_argument("--out", default="bench/beads-vs-aidememo/data")
    args = ap.parse_args()
    ...
    aidememo_path = out_dir / "corpus_aidememo.jsonl"
    bd_path = out_dir / "corpus_beads.jsonl"

    with aidememo_path.open("w") as f:
        for r in records:
            # aidememo fact body = title + "\n" + description (a representative
    ...
    print(f"  {aidememo_path} ({aidememo_path.stat().st_size / 1024:.1f} KiB)")
