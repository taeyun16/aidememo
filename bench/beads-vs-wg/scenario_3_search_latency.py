#!/usr/bin/env python3
"""Scenario #3 — single-query search latency, wg vs beads.
...
Caveats:
  - wg's hybrid search is BM25 + (optional) embeddings. We turn off
...
def main() -> int:
    ...
    subprocess.run([WG, "--store", WG_STORE, "search", queries[0], "--json", "-l", "10"],
                   capture_output=True)
    subprocess.run([BD, "search", queries[0], "--json", "--limit", "10"],
                   cwd=BD_DIR, capture_output=True)

    wg_cmds = [[WG, "--store", WG_STORE, "search", q, "--json", "-l", "10"] for q in queries]
    bd_cmds = [[BD, "search", q, "--json", "--limit", "10"] for q in queries]

    print(f"# measuring wg…", file=sys.stderr)
    wg_res = measure("wg search", wg_cmds)
    print(f"#   p50={wg_res['p50_ms']:.1f}ms p95={wg_res['p95_ms']:.1f}ms",
          file=sys.stderr)
    ...
    out = {
        ...
        "beads": bd_res,
        "comparison": {
            "p50_ratio_wg_over_beads": wg_res["p50_ms"] / bd_res["p50_ms"]
                if bd_res["p50_ms"] > 0 else None,
            "p95_ratio_wg_over_beads": wg_res["p95_ms"] / bd_res["p95_ms"]
