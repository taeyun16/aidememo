#!/usr/bin/env python3
"""Scenario #1 — bulk write throughput, wg vs beads.

Inputs: corpus_wg.jsonl, corpus_beads.jsonl (run gen.py first).

For each tool we measure: wall time, throughput (records/s), and
final on-disk size of the data directory.

  - wg side    : invokes the `wg_fact_add_many` MCP tool ONE TIME
                 with all N records as a JSON array. This is wg's
...
DATA = Path(os.environ.get("BENCH_DATA", "bench/beads-vs-wg/data"))
RESULTS = Path(os.environ.get("BENCH_RESULTS", "bench/beads-vs-wg/results"))
...
def wg_bulk_via_mcp(corpus: Path, store: Path) -> dict:
    """Issue ONE wg_fact_add_many MCP call with all N records."""
    items = []
    ...
    call = {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
            "params": {"name": "wg_fact_add_many", "arguments": {"items": items}}}
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"wg mcp failed: {proc.stderr.strip()[:300]}")
...
def main() -> int:
    wg_corpus = DATA / "corpus_wg.jsonl"
    bd_corpus = DATA / "corpus_beads.jsonl"
    if not wg_corpus.exists() or not bd_corpus.exists():
        print(f"missing corpus — run gen.py first", file=sys.stderr)
        return 2

    n_records = sum(1 for line in wg_corpus.read_text().splitlines() if line.strip())

    wg_dir = Path("/tmp/wg-vs-beads/wg")
    bd_dir = Path("/tmp/wg-vs-beads/beads")
    reset_dir(wg_dir)
    reset_dir(bd_dir)
    wg_store = wg_dir / "wiki.redb"

    print(f"# wg bulk insert (N={n_records}) via wg_fact_add_many…",
          file=sys.stderr)
    wg_res = wg_bulk_via_mcp(wg_corpus, wg_store)
    wg_res["disk_bytes"] = du_bytes(wg_dir)
    ...
    out = {
        ...
        "beads": bd_res,
        "comparison": {
            "wall_speedup_wg_over_beads": bd_res["wall_ms"] / wg_res["wall_ms"]
                if wg_res["wall_ms"] > 0 else None,
            "disk_ratio_wg_over_beads": wg_res["disk_bytes"] / bd_res["disk_bytes"]
