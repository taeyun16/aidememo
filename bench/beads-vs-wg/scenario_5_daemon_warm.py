...
Adds two daemon-mode rows to the scenario #4 grid:
  4. wg --via --bm25 warm           (BM25 over HTTP, model not loaded)
  5. wg --via (hybrid HNSW) warm    (HNSW over HTTP, model warm)

Daemon spawns a single `wg mcp-serve` background process before the
...
Comparison anchors:
  - wg --bm25 (local, fresh CLI)         from scenario #4 = ~71ms
  - bd search                             from scenario #4 = ~387ms
  - wg (HNSW hybrid, local fresh CLI)     from scenario #4 = ~850ms
"""
...
def main() -> int:
    ...
    log_path = Path("/tmp/wg-vs-beads/scenario5-daemon.log")
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_fp = open(log_path, "w")
    server = subprocess.Popen(
        [WG, "mcp-serve", "--port", str(port), WG_STORE],
    ...
    try:
        ...
        m_via_hybrid = measure(
            "wg --via hybrid (HNSW, warm daemon)",
            queries,
    ...
    out = {
        ...
        "anchors_from_scenario_4": {
            "wg --bm25 (local fresh)": "p50 ~71 ms",
            "bd search":                "p50 ~387 ms",
            "wg HNSW hybrid (local fresh)": "p50 ~850 ms",
        },
    }
