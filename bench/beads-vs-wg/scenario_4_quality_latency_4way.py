...
Modes:
  1. wg --bm25      : new lazy fast path. No model load.
  2. wg (default)   : hybrid_search; with HNSW sidecar present uses
                      hybrid_search_with_hnsw, otherwise BM25 fallback
                      that still embeds the query.
  3. wg --no-hnsw   : forces the BM25-fallback hybrid path. Lets us
...
WG_STORE = "/tmp/wg-vs-beads/wg/wiki.redb"
WG_HNSW = Path("/tmp/wg-vs-beads/wg/wiki.hnsw.bin")
BD_DIR = "/tmp/wg-vs-beads/beads"
DATA = Path("bench/beads-vs-wg/data")
RESULTS = Path("bench/beads-vs-wg/results")
...
def parse_wg_ids(stdout: str) -> list[str]:
    try:
...
def main() -> int:
    queries = sample_queries(DATA / "corpus_wg.jsonl", N_QUERIES)
    print(f"# {len(queries)} queries", file=sys.stderr)

    has_sidecar = WG_HNSW.exists()
    print(f"# HNSW sidecar present: {has_sidecar}", file=sys.stderr)
    if not has_sidecar:
        print("# build sidecar first: wg --store … vector-rebuild", file=sys.stderr)
        return 2

    # warmup
    subprocess.run([WG, "--store", WG_STORE, "search", queries[0],
    ...
    print("# wg --bm25…", file=sys.stderr)
    m1 = measure_mode(
        "wg --bm25",
        queries,
        lambda q: [WG, "--store", WG_STORE, "search", q, "--bm25", "--json", "-l", "5"],
        parse_wg_ids,
    )
    ...
    print("# wg (HNSW hybrid)…", file=sys.stderr)
    m2 = measure_mode(
        "wg (HNSW hybrid)",
        queries,
        lambda q: [WG, "--store", WG_STORE, "search", q, "--json", "-l", "5"],
        parse_wg_ids,
    )
    ...
    backup = WG_HNSW.with_suffix(".bin.hidden")
    shutil.move(str(WG_HNSW), str(backup))
    try:
        print("# wg (BM25-fallback hybrid, sidecar hidden)…", file=sys.stderr)
        m3 = measure_mode(
            "wg (hybrid, no HNSW)",
            queries,
            lambda q: [WG, "--store", WG_STORE, "search", q, "--json", "-l", "5"],
        ...
        shutil.move(str(backup), str(WG_HNSW))
