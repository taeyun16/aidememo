...
| LongMemEval retrieval ceiling | Mem0 / Zep / Mastra don't publish R@K — only LLM-graded E2E. | **wg ships R@10 = 0.986, R@1 = 0.938, MRR = 0.957** with `bge-small-en-v1.5` + `bge-reranker-base` (both via `fastembed` ONNX, in-process). 469/500 questions land the gold evidence at the FIRST retrieved slot. The cross-encoder reranker is now wired the same way OMEGA wires it (in-process, no service). |
