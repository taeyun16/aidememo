...
| Conversation auto-extract | Yes (LLM-driven) | Heuristic / agent-assisted (`wg_extract`, `fact_add_many`) |
...
|---|---|---|
| LLM-grade auto-extraction | Mastra Observer/Reflector and Supermemory pre-curate chat at insert time with an LLM, hitting 84-95% on LongMemEval. wg's default `wg_extract` path is heuristic. | Agent can call its own LLM and feed structured output to `wg_fact_add_many`; wg keeps this explicit so the default path stays local-first and zero-token. |
| LongMemEval E2E SOTA | OMEGA reports 95.4% with gpt-4.1, deep hooks, rerank, and temporal prompting. | wg @ bge + reranker K=20→10 measured 74.0% with MiniMax-M2.7-highspeed and 72.6% with gpt-4.1. Retrieval is high (R@10 = 0.992); remaining errors are mostly reader-side temporal / multi-session reasoning. |
| Multi-writer merge | beads has git-style cell merge; wg is single-writer. | Use `lock_retry_ms` for small same-host teams, one `wg mcp-serve` daemon for shared writes, and pull-only sync for local read caches. Full distributed write merge isn't in scope. |
| Community / cluster detection | Graphiti groups related entities into communities. | Not implemented; out of scope for v0.x. |
| Cloud-managed multi-tenant ops | SaaS peers partition users and operate the service for you. | `source_id` scopes facts/search inside one store and projects provide hard local isolation, but wg does not try to be hosted multi-tenant infrastructure. |
