...
def _handler_factory(client: WgClient):
...
def register(ctx: Any, client: WgClient) -> None:
    ctx.register_cli_command(
        name="wg",
        help="Wiki-Graph subcommands (query / search / recent / add / stats / lint)",
        setup_fn=_setup,
        handler_fn=_handler_factory(client),
        description="Talk to the wg knowledge graph from the hermes CLI.",
    )
