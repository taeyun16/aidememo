...
compatibility: Requires the `aidememo` CLI binary on PATH. Public v0.1.0 supports the baseline plugin; current-main handoff tools require a matching CLI built from https://github.com/taeyun16/aidememo. Optionally registers as an MCP server (`aidememo mcp` for stdio, `aidememo mcp-serve` for HTTP).
