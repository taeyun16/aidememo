...
``~/.hermes/state/aidememo-pending.jsonl``) and uses one JSON object per
...
def pending_log_path() -> Path:
    ...
    if env_state:
        return Path(env_state) / "aidememo-pending.jsonl"
    env_home = os.environ.get("HERMES_HOME")
    if env_home:
        return Path(env_home) / "state" / "aidememo-pending.jsonl"
    return Path.home() / ".hermes" / "state" / "aidememo-pending.jsonl"
...
def commit_one(client: AideMemoClient, idx: int, path: Path | None = None) -> PendingEntry:
    """Commit a single entry to aidememo and remove it from the log.
...
def commit_all(client: AideMemoClient, path: Path | None = None) -> tuple[int, list[PendingEntry]]:
    """Commit every entry to aidememo and clear the log on success.
    ...
    failure (aidememo unreachable, validation error from one entry) leaves
...
def clear_one(idx: int, path: Path | None = None) -> PendingEntry:
    """Drop a single entry without writing to aidememo.
...
def _renumber(entries: Iterable[PendingEntry]) -> list[PendingEntry]:
    ...
    what the user sees on the next ``/aidememo-pending``."""
    return [
        PendingEntry(
            idx=i,
            ts_ms=e.ts_ms,
