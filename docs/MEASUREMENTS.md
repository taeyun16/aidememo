...
|---|---|
| `scripts/release-preflight.sh` | One-command release gate with timed summary rows. `local` profile runs version gate, workflow syntax lint when `actionlint` is available, binding release smoke, and workflow release smoke; `full` profile adds optional Python/Elixir/C binding smokes plus Python/npm publish dry-runs. Latest local profile: version `0.16s`, actionlint `0.11s`, binding smoke `49.91s`, workflow smoke `15.85s`, total `66.03s`; fast path with bindings/workflow disabled passed version + actionlint in `0.35s` and recorded skipped heavy steps explicitly. |
...
| `scripts/wg-python-pack-smoke.sh` | Builds a `wg-python` wheel with maturin for a temp venv interpreter, installs that wheel into the venv, runs `crates/wg-python/tests/smoke.py`, and verifies installed wheel metadata equals `wg_python.__version__`. Local macOS arm64: built `wg_python-0.1.0-cp313-cp313-macosx_11_0_arm64.whl`, smoke passed including `workflow_start(..., bm25_only=True)`, installed version `0.1.0`. |
| `scripts/wg-python-publish-dry-run.sh` | Builds PyPI publish payloads without uploading: one wheel plus one sdist, then validates package name/version metadata, native module presence, required sdist files, and forbidden build artifacts. Local macOS arm64: built `wg_python-0.1.0-cp313-cp313-macosx_11_0_arm64.whl` + `wg_python-0.1.0.tar.gz`; the dry-run rebuilds the wheel from the sdist, proving the vendored `tokenizers` patch is present. |
...
| `scripts/wg-napi-pack-smoke.sh` | Builds release addon, runs `npm test`, packs root `wg-napi`, packs the current platform package, then installs both tarballs into a temp project with offline/no-audit npm flags and verifies `require("wg-napi").version()`. Local macOS arm64: root `wg-napi-0.1.0.tgz` is 3.97 KB / 4 files / includes README + no `.node`; platform `wg-napi-darwin-arm64-0.1.0.tgz` is 2.79 MB / 2 files / includes `wg-napi.darwin-arm64.node`; smoke includes `workflowStart(..., bm25Only:true)`. |
...
| `scripts/wg-napi-publish-dry-run.sh` | Wrapper around the publish engine with `WG_NAPI_PUBLISH_MODE=dry-run`. Local macOS arm64: root payload `wg-napi@0.1.0`, 4 files, 3.97 KB packed, README included, no `.node`; platform payload `wg-napi-darwin-arm64@0.1.0`, 2 files, 2.79 MB packed. |
