...
| `scripts/ci-local.sh demo` | Local first-run workflow smoke. `bash -n scripts/ci-local.sh` passes; `scripts/ci-local.sh demo` recovered decision=1, lesson=1, error=1, search_hits=4, workflow latency 117ms, wall 0.68s. `scripts/ci-local.sh all` now runs this check between lint and SDK promotion. |
