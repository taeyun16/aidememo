...
| `scripts/cargo-package-readiness.sh` | Rust publish-order package gate. Runs the release version gate, verifies `cargo package -p aidememo-core`, and records dependent Rust crates as a deliberate skip until `aidememo-core` is published to crates.io at the matching version. Latest local run: version gate 0.15s, `cargo package aidememo-core` 3.51s, packaged 38 files / 1005.4KiB raw / 223.1KiB compressed, dependent crates skipped with `AIDEMEMO_CARGO_PACKAGE_CHECK_DEPENDENTS=1` guidance, total 3.66s. |
