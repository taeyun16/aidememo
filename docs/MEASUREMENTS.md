...
`benchmarks/*/RESULTS.md`, or in committed JSON under `bench/**/evidence`.
...
| MCP install source/actor/backend defaults | `cargo test -p aidememo-cli --bin aidememo mcp_install`: 12/12 tests passed; install reports and generated config carry `source_id`, `actor_id`, the selected backend, `AIDEMEMO_SOURCE_ID`, and `AIDEMEMO_ACTOR_ID` | `mcp-install --source-id <scope> --actor-id <installation>` makes project scope and multi-account routing explicit without hand-editing each agent config. |
...
| MCP install source/actor/backend defaults Scenario M | 28/28 invariants; elapsed 76.43ms; Codex / Cursor / OpenCode configs contain source, per-installation actor alias, and `--backend libsqlite`; Claude / Hermes / OpenClaw print commands include both env values; MCP write/search and inbox calls consume installed defaults | Generated configs provide both the retrieval boundary and account/installation address used by the handoff pull flow. |
...
| Cross-agent handoff Scenario P | 12/12 quality gates and 3/3 efficiency gates; critical decision/lesson/error/question recall 4/4; route fields 4/4; neighbouring-source leakage 0; CLI/MCP/SDK content parity; structured SDK packet, observable `done_when`, compact `AGENT/PROFILE` routes, and one-command receiver resume preserved; raw thread 33,640 bytes / ~8,410 tokens, canvas 9,055 / ~2,264, handoff 5,784 / ~1,446 (`-82.8%` vs raw, `-36.1%` vs canvas); CLI/MCP/SDK export 62.96ms | The deterministic handoff protocol preserves the preregistered evidence, completion condition, routing, and structured consumption contract while reducing context. This is zero-token protocol evidence, not a downstream model task-success result. The first run failed critical recall 0/4 because bounded artifacts selected the oldest fact window; the fix keeps global paginated `fact_list` order unchanged while selecting the latest attached session window before chronological rendering. |
| Multi-account handoff Scenario Q | 11/11 gates; three independent MCP processes as `codex-one`, `codex-two`, `claude-main`; `codex-one -> codex-two -> codex-one` round trip on one session; actor/source leakage 0; one pointer entity and zero facts added per dispatch; persisted broker/payload keys 0; elapsed 250.62ms | Two subscriptions of the same agent and a third vendor can route one workflow without exchanging vendor-local chat ids. This proves bidirectional pull/acknowledgement and pointer-only persistence, not authenticated identity, exclusive locking, exactly-once delivery, or model task success. |
| Hermes Kanban boundary Scenario R | 13/13 gates on Hermes Agent v0.18.2 using a real temporary Kanban DB; plugin-derived `hermes:board:default:tenant:release-team` scope and `hermes:orchestrator` actor; card state `ready/coding -> ready/reviewer -> done/reviewer`; internal profile transition created zero AideMemo assignments; external `codex-two` dispatch added one pointer entity and zero facts; returned evidence stayed on the same session; elapsed 1.18s | Hermes Kanban remains the canonical lifecycle for claims, profile routing, retries, and completion, while AideMemo bridges durable evidence only at the external installation boundary. Board/tenant mapping is trusted-process routing, not gateway authentication. This is zero-token protocol evidence, not an implemented external CLI worker spawner or model task-success result. |
| External worker lane Scenario S | 16/16 gates with fake zero-token Codex and Claude CLIs; automatic workers persisted distinct exclusive claim tokens and a failed assignment accepted a new worker claim with `attempt_count=2`; Codex success received shell-free defaults, workspace, packet, and resume environment, returned a result on the same session, then completed the acknowledgement; Claude exit 7 returned an error on the same session and left the assignment accepted; sender outbox/status linked both result facts; elapsed 1.03s | The installable receiver contract now covers process invocation and fact-linked return evidence without claiming live-model task success. It does not provide authentication, automatic retry, exactly-once execution, Hermes `spawn_fn` registration, or Kanban lifecycle mutation. |
| aidememo-agent-sdk wheel install smoke | `scripts/aidememo-agent-sdk-pack-smoke.sh`: built `aidememo_agent_sdk-0.1.0-py3-none-any.whl`, installed it into a temp venv, verified `Memory`, `AideMemoClient`, `AideMemoMemorySDK`, `WorkerLaneConfig`, `WorkerLaneResult`, and the installed `aidememo-worker-lane --help` entry point; total 3.28s | The code-first SDK and external receiver are installable independently of Hermes, so Codex / Claude Code / CI scripts do not need the Hermes plugin package. |
...
Scenario M and P-S values above are the 2026-07-26 macOS arm64 snapshot at
commit `36e8ba8`; the compact environment and result record is
[`bench/multi-agent/evidence/handoff-main-2026-07-26.json`](../bench/multi-agent/evidence/handoff-main-2026-07-26.json).
...
|---|---|
| `scripts/release-preflight.sh` | One-command release gate with timed summary rows. `local` profile runs version gate, changelog release gate, registry readiness gate, public portability gate, workflow syntax lint when `actionlint` is available, docs feature gate/build, binding release smoke, workflow release smoke, and SDK promotion check; the binding smoke now includes NAPI pack/install, `aidememo-agent-sdk` wheel install, and `hermes-aidememo` wheel install by default. `full` profile adds Rust publish dry-run readiness, optional Python/Elixir/C binding smokes, plus `aidememo-python`, `aidememo-agent-sdk`, `hermes-aidememo`, and `aidememo-napi` publish dry-runs. Child scripts still print their own summaries to stdout, but preflight clears child `$GITHUB_STEP_SUMMARY` so CI gets one top-level `release-preflight` table. Binding-only measured path (`AIDEMEMO_RELEASE_PREFLIGHT_CHANGELOG=0 AIDEMEMO_RELEASE_PREFLIGHT_ACTIONLINT=0 AIDEMEMO_RELEASE_PREFLIGHT_DOCS=0 AIDEMEMO_RELEASE_PREFLIGHT_STORAGE_BACKEND=0 AIDEMEMO_RELEASE_PREFLIGHT_WORKFLOW=0 AIDEMEMO_RELEASE_PREFLIGHT_SDK_PROMOTION=0 AIDEMEMO_RELEASE_PREFLIGHT_PUBLISH=0`): version 0.14s, binding smoke 53.13s, total 53.27s. Publish-only full-profile path (`AIDEMEMO_RELEASE_PREFLIGHT_PROFILE=full AIDEMEMO_RELEASE_PREFLIGHT_CHANGELOG=0 AIDEMEMO_RELEASE_PREFLIGHT_ACTIONLINT=0 AIDEMEMO_RELEASE_PREFLIGHT_DOCS=0 AIDEMEMO_RELEASE_PREFLIGHT_STORAGE_BACKEND=0 AIDEMEMO_RELEASE_PREFLIGHT_BINDINGS=0 AIDEMEMO_RELEASE_PREFLIGHT_WORKFLOW=0 AIDEMEMO_RELEASE_PREFLIGHT_SDK_PROMOTION=0 AIDEMEMO_RELEASE_PREFLIGHT_PUBLISH=1`): version 0.13s, `aidememo-python` publish 56.19s, agent SDK publish 3.86s, Hermes publish 3.86s, NAPI publish 4.05s, total 68.09s. Narrow full-profile cargo-package integration path (`AIDEMEMO_RELEASE_PREFLIGHT_PROFILE=full AIDEMEMO_RELEASE_PREFLIGHT_CHANGELOG=0 AIDEMEMO_RELEASE_PREFLIGHT_ACTIONLINT=0 AIDEMEMO_RELEASE_PREFLIGHT_DOCS=0 AIDEMEMO_RELEASE_PREFLIGHT_STORAGE_BACKEND=0 AIDEMEMO_RELEASE_PREFLIGHT_BINDINGS=0 AIDEMEMO_RELEASE_PREFLIGHT_WORKFLOW=0 AIDEMEMO_RELEASE_PREFLIGHT_SDK_PROMOTION=0 AIDEMEMO_RELEASE_PREFLIGHT_CARGO_PACKAGE=1 AIDEMEMO_RELEASE_PREFLIGHT_PUBLISH=0`): version 0.17s, registry readiness 0.04s, Rust publish dry-run readiness 3.58s, total 3.79s. `AIDEMEMO_RELEASE_PREFLIGHT_SDK_REQUIRE_PUBLIC=1` fails the SDK step with exit 1 while recording the failure row. Forced `AIDEMEMO_RELEASE_PREFLIGHT_PROFILE=full AIDEMEMO_RELEASE_PREFLIGHT_ACTIONLINT_BIN=/nonexistent/actionlint` records `fail | workflow syntax lint | 0.00 | /nonexistent/actionlint not installed`. Manual runner-backed execution is available through `.github/workflows/release-preflight.yml`. |
| `.github/workflows/release-preflight.yml` | Manual pre-publish workflow for `scripts/release-preflight.sh`. It takes `version`, `local`/`full` profile, Rust package readiness, publish dry-run, optional binding, SDK smoke, Scenario K, and public-SDK requirement toggles; installs Rust 1.96.0, Python 3.13, Node 24, `uv==0.11.21`, actionlint, and Docusaurus dependencies before running the same release gate. This keeps the heavy release-candidate check out of normal PR CI while making it reproducible in GitHub Actions before publish. |
| `scripts/changelog-release-check.py` | Offline changelog release gate. Verifies `CHANGELOG.md` has exactly one empty `[Unreleased]` section, a dated current-version section immediately below it, and non-empty release-note content. Latest local run: version=0.1.0, date=2026-07-09. |
| `scripts/registry-readiness-check.py` | Offline registry mapping gate. Verifies PyPI project names, workflow names, `pypi-publish` / `npm-publish` environments, OIDC `id-token: write` publish permissions, npm root/platform package graph, cargo-package CI wiring, public-registry smoke, release-preflight, fresh-checkout workflow, portability-gate wiring, release-doc package lists, and rejects long-lived publish-token assumptions in first-party publish workflows. |
| `scripts/cargo-package-readiness.sh` | Rust publish-order dry-run gate. Runs the release version gate, verifies `cargo publish --dry-run -p aidememo-core`, and records dependent Rust crates as a deliberate skip until `aidememo-core` is published to crates.io at the matching version. Latest local run: version gate 0.17s, `cargo publish --dry-run aidememo-core` 3.49s, packaged 38 files / 1005.4KiB raw / 223.1KiB compressed, reached the upload step, then aborted because of dry-run, total 3.66s. |
| `scripts/public-portability-check.py` | Offline public-source gate. Scans first-party tracked text files and rejects developer-specific macOS, Linux, or Windows home paths. It runs in normal CI and immediately after registry readiness in release preflight. |
| `scripts/fresh-checkout-smoke.sh` | Public onboarding smoke. Copies the current checkout to a temp directory without `.git`, `target`, Docusaurus build output, or Node build artifacts, verifies `install.sh` syntax, builds `aidememo-cli`, then runs deterministic quickstart `fact add`, `search --bm25-only`, `query --bm25-only`, `workflow start --bm25-only`, and `stats`. Latest local run: copy 1.09s, build 24.59s, help + quickstart commands 0.23s, total 25.93s. |
| `.github/workflows/fresh-checkout-smoke.yml` | Ubuntu public-onboarding workflow. Runs the same fresh-copy build and deterministic quickstart manually or on pull requests that change the installer, CLI, core, or smoke contract. |
| `scripts/public-registry-smoke.sh` | Post-release public install smoke. Default `plan` mode is safe before packages exist and records the exact install checks for `aidememo-cli`, `aidememo-agent-sdk`, `aidememo-agent-sdk[binding]` + `aidememo-python`, `hermes-aidememo`, and `aidememo-napi`; latest plan run marked all 5 checks `ready` in 0.00s for version 0.1.0. After real publishes, run `AIDEMEMO_PUBLIC_REGISTRY_SMOKE_MODE=verify scripts/public-registry-smoke.sh` or the manual `.github/workflows/public-registry-smoke.yml` workflow to install from crates.io, PyPI, and npm into temp environments and import / execute the installed packages. |
| `.github/workflows/public-registry-smoke.yml` | Manual post-release smoke workflow. Takes the exact `version`, `plan`/`verify` mode, and per-registry toggles, then runs `scripts/public-registry-smoke.sh` on Ubuntu with Rust 1.96.0, Python 3.13, and Node 24. It keeps live registry verification separate from PR CI while making the same check reproducible from GitHub after publish. |
| CI `cargo-package-readiness` job | Pull-request guard for the Rust publish dry-run gate. Runs after `lint` on Ubuntu with Rust 1.96.0, Python 3.13, Node 24, and `scripts/cargo-package-readiness.sh`; sets `AIDEMEMO_CARGO_PACKAGE_CHECK_DEPENDENTS=0` so `aidememo-core` `cargo publish --dry-run` is enforced while dependent crates remain a documented publish-order skip until `aidememo-core` is live on crates.io. |
| CI `sdk-promotion-check` job | Runs `scripts/sdk-promotion-check.sh` on Ubuntu after lint with Python 3.13 and Node 22, keeping SDK wording and binding surface drift visible in PR checks without running package smokes. Local workflow lint: `actionlint .github/workflows/*.yml` 0 issues; local gate: ok=22, ready=3, blocked=2, fail=0. |
...
| `scripts/sdk-promotion-check.sh` | Package-SDK wording and parity gate for `aidememo-python`, `aidememo-napi`, and `aidememo-agent-sdk`. After public PyPI/npm verification: ok=24, ready=3, blocked=0, fail=0, `local_ready=true`, `sdk_promotable=true`; `AIDEMEMO_PUBLIC_REGISTRY_SMOKE_MODE=verify mise run public-registry-smoke` installed and imported the public Python and Node packages. The gate explicitly checks session-aware writes and pinned context API/docs for Python, Node, and the agent SDK. With `AIDEMEMO_SDK_PROMOTION_RUN_SCENARIO_K=1`, the script self-provisions a temp `aidememo-python` wheel/venv and rebuilds the Node addon before running Scenario K; latest local Scenario K run: 8/8 invariants, p50 CLI 8342.57ms, Python 2.32ms, Node 2.64ms, leakage 0. Release preflight runs this gate by default; CI gets the same table in `$GITHUB_STEP_SUMMARY`; set `AIDEMEMO_RELEASE_PREFLIGHT_SDK_PROMOTION=0` only for focused debugging. |
