...
| `scripts/registry-readiness-check.py` | Offline registry mapping gate. Verifies PyPI project names, workflow names, `pypi-publish` / `npm-publish` environments, OIDC `id-token: write` publish permissions, npm root/platform package graph, cargo-package CI wiring, release-doc package lists, and rejects long-lived publish-token assumptions in first-party publish workflows. Latest local run: pypi=3, npm=6, cargo-package-ci, docs/workflows aligned. |
| `scripts/cargo-package-readiness.sh` | Rust publish-order package gate. Runs the release version gate, verifies `cargo package -p aidememo-core`, and records dependent Rust crates as a deliberate skip until `aidememo-core` is published to crates.io at the matching version. Latest local run: version gate 0.15s, `cargo package aidememo-core` 3.57s, packaged 38 files / 1005.4KiB raw / 223.1KiB compressed, dependent crates skipped with `AIDEMEMO_CARGO_PACKAGE_CHECK_DEPENDENTS=1` guidance, total 3.72s. |
...
| CI `cargo-package-readiness` job | Pull-request guard for the Rust package gate. Runs after `lint` on Ubuntu with Rust 1.96.0, Python 3.13, Node 24, and `scripts/cargo-package-readiness.sh`; sets `AIDEMEMO_CARGO_PACKAGE_CHECK_DEPENDENTS=0` so `aidememo-core` packageability is enforced while dependent crates remain a documented publish-order skip until `aidememo-core` is live on crates.io. |
