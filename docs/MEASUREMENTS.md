...
| `scripts/registry-readiness-check.py` | Offline registry mapping gate. Verifies PyPI project names, workflow names, `pypi-publish` / `npm-publish` environments, OIDC `id-token: write` publish permissions, npm root/platform package graph, cargo-package CI wiring, public-registry smoke wiring, release-doc package lists, and rejects long-lived publish-token assumptions in first-party publish workflows. Latest local run: pypi=3, npm=6, cargo-package-ci, public-registry-smoke, docs/workflows aligned. |
...
| `scripts/public-registry-smoke.sh` | Post-release public install smoke. Default `plan` mode is safe before packages exist and records the exact install checks for `aidememo-cli`, `aidememo-agent-sdk`, `aidememo-agent-sdk[binding]` + `aidememo-python`, `hermes-aidememo`, and `aidememo-napi`; latest plan run marked all 5 checks `ready` in 0.00s for version 0.1.0. After real publishes, run `AIDEMEMO_PUBLIC_REGISTRY_SMOKE_MODE=verify scripts/public-registry-smoke.sh` to install from crates.io, PyPI, and npm into temp environments and import / execute the installed packages. |
