...
| Multi-account handoff Scenario Q | 10/10 gates; three independent MCP processes as `codex-one`, `codex-two`, `claude-main`; actor/source leakage 0; one pointer entity and zero facts added per dispatch; accepted receiver fact visible in the same session; persisted broker/payload keys 0; elapsed 287.57ms | Two subscriptions of the same agent and a third vendor can route one workflow without exchanging vendor-local chat ids. This proves pull/acknowledgement and pointer-only persistence, not authenticated identity, exclusive locking, exactly-once delivery, or model task success. |
| Hermes Kanban boundary Scenario R | 12/12 gates on Hermes Agent v0.18.2 using a real temporary Kanban DB; card state `ready/coding -> ready/reviewer -> done/reviewer`; internal profile transition created zero AideMemo assignments; external `codex-two` dispatch added one pointer entity and zero facts; returned evidence stayed on the same session; elapsed 1.33s | Hermes Kanban remains the canonical lifecycle for claims, profile routing, retries, and completion, while AideMemo bridges durable evidence only at the external installation boundary. This is zero-token protocol evidence, not an implemented external CLI worker spawner, authenticated identity, or model task-success result. |
