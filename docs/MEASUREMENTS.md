...
| `scripts/registry-readiness-check.py` | Offline registry mapping gate. Verifies PyPI project names, workflow names, `pypi-publish` / `npm-publish` environments, OIDC `id-token: write` publish permissions, npm root/platform package graph, cargo-package CI wiring, public-registry smoke script/workflow wiring, release-preflight workflow wiring, release-doc package lists, and rejects long-lived publish-token assumptions in first-party publish workflows. Latest local run: pypi=3, npm=6, cargo-package-ci, public-registry-smoke, release-preflight-workflow, docs/workflows aligned. |
