...
| `scripts/sdk-promotion-check.sh` | Package-SDK wording and parity gate for `aidememo-python`, `aidememo-napi`, and `aidememo-agent-sdk`. Default local run through release preflight: ok=22, ready=3, blocked=2, fail=0, `local_ready=true`, `sdk_promotable=false` because public PyPI/npm installs are not verified. The gate explicitly checks session-aware writes and pinned context API/docs for Python, Node, and the agent SDK. This does not block positioning `aidememo-agent-sdk` as the agent-facing SDK path. With `AIDEMEMO_SDK_PROMOTION_RUN_SCENARIO_K=1`, the script now self-provisions a temp `aidememo-python` wheel/venv and rebuilds the Node addon before running Scenario K; latest local Scenario K run: 8/8 invariants, p50 CLI 8342.57ms, Python 2.32ms, Node 2.64ms, leakage 0. Release preflight runs this gate by default; CI gets the same table in `$GITHUB_STEP_SUMMARY`; set `AIDEMEMO_RELEASE_PREFLIGHT_SDK_PROMOTION=0` only for focused debugging. |
