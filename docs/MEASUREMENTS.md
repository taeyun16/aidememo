...
| `scripts/hermes-aidememo-publish-dry-run.sh` | Builds pure-Python PyPI payloads without uploading and writes a timed Markdown summary. Local macOS arm64: venv 1.63s, build backend install 0.54s, `python -m build --wheel --sdist` 1.51s, payload validation 0.06s, total 3.74s; built `hermes_aidememo-1.0.0-py3-none-any.whl` + `hermes_aidememo-1.0.0.tar.gz`. Validator checks metadata, `aidememo-agent-sdk` and `PyYAML` dependency declarations, `plugin.yaml`, bundled skill files, and cache/build artifact exclusion. |
