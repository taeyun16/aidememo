...
| Server / SSOT foundation | `crates/aidememo-domain`, `aidememo-service`, `aidememo-store-local`, and the bounded `aidememo-server` HTTP resource API; PostgreSQL, artifact, client/replica, and product-domain APIs remain proposed | [`Server and SSOT Architecture`](SERVER_SSOT.md) |
