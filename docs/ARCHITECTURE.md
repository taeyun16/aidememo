...
| Server / SSOT foundation | `crates/aidememo-domain`, `aidememo-service`, `aidememo-store-local`, and `aidememo-server`; bounded typed session/fact/handoff HTTP, transactional actor mailbox indexes, and raw `custom.*` resources exist, while PostgreSQL, artifacts, search, and client/replica APIs remain proposed | [`Server and SSOT Architecture`](SERVER_SSOT.md) |
