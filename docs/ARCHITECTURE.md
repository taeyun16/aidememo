...
| Server / SSOT foundation | `crates/aidememo-domain`, `aidememo-service`, `aidememo-store-local`, `aidememo-server`, and `aidememo-client`; bounded typed session/fact/handoff HTTP, transactional actor mailbox indexes, raw `custom.*` resources, and an isolated exact-read replica exist, while PostgreSQL, artifacts, retrieval projection, HTTP MCP gateway routing, and offline writes remain proposed | [`Server and SSOT Architecture`](SERVER_SSOT.md) |
