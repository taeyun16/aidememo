# SkillOpt-lite For wg

`wg` borrows the useful part of SkillOpt without making runtime memory
...
The artifact is the procedure that tells agents how to use `wg` memory:

- `wg-skill/SKILL.md` for model-visible agent instructions;
- `wg-skill/reader-prompts.md` for reader prompt snippets;
- `packages/wg-agent-sdk/README.md` examples for code-first memory;
- a future generated `best_skill.md` / `SKILL.md`-compatible profile copied
  into the right agent skill location by `wg skill install`.

The artifact should teach when to use:

- `wg_workflow_start` for sparse tickets and PR/issue automation;
- `wg_context` for the top of a normal turn;
- `wg_query` / `wg_search` for narrower follow-up retrieval;
- `wg_aggregate` only for counts, totals, date sets, and timelines;
...
Avoid broad rewrites that change many workflows at once. Keep `wg_aggregate`
...
`cargo check -p wg-cli`, the zero-token workflow demo, and the SDK promotion
gate. To validate a candidate file:

```bash
WG_SKILLOPT_CANDIDATE=/tmp/SKILL.md scripts/skillopt-lite-check.sh
```
...
```bash
WG_SKILLOPT_RUN_SCENARIOS=1 scripts/skillopt-lite-check.sh
```
...
`lesson` facts with entities `wg`, `SkillOptLite`, and the affected workflow.
...
The important SkillOpt lesson for `wg` is the discipline, not the benchmark
