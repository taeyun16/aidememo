...
|---|---|---|
| `wg-python` | SDK candidate | Hermes already gets the largest measured lift from in-process Python: Scenario G showed p50 `1795.71ms` CLI vs `13.14ms` binding with shape parity. Python now exposes `workflow_start` and typed exceptions, so the remaining blocker is public PyPI install. |
| `wg-napi` | SDK candidate | Node has platform package split, npm pack/install smoke, dry-run publish gates, a native adapter path that was `1.66x` faster than daemon BrainBench on the same checkout, a `workflowStart` API, package README examples, and stable `[wg_code]` error messages. The remaining blocker is public npm install. |
