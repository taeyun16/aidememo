...
use aidememo_core::{
    AideMemo, AideMemoError, EntityRecord, EntitySummary, FactRecord, LintReport, QueryResult,
    Result, SearchResult, StoreStats, TraverseResult,
};
...
pub fn format_entity(entity: &EntityRecord, format: Format) -> Result<String> {
    match format {
        Format::Json => {
            serde_json::to_string_pretty(entity).map_err(|e| AideMemoError::Serialize {
...
pub fn format_entity_show(
    ...
    match &entity.summary {
        ...
        None => {
            out.push_str("Summary: (none — set with `aidememo entity describe <name> \"...\"`)\n");
        }
    }
    out.push('\n');
...
pub fn format_entity_list(entities: &[EntitySummary], format: Format) -> Result<String> {
    match format {
        Format::Json => {
            serde_json::to_string_pretty(entities).map_err(|e| AideMemoError::Serialize {
...
pub fn format_fact(fact: &FactRecord, wiki: &AideMemo, format: Format) -> Result<String> {
    if let Format::Json = format {
        return serde_json::to_string_pretty(fact).map_err(|e| AideMemoError::Serialize {
...
pub fn format_fact_list(facts: &[FactRecord], wiki: &AideMemo, format: Format) -> Result<String> {
    if let Format::Json = format {
        return serde_json::to_string_pretty(facts).map_err(|e| AideMemoError::Serialize {
...
pub fn format_traverse(result: &TraverseResult, format: Format) -> Result<String> {
    match format {
        Format::Json => {
            serde_json::to_string_pretty(result).map_err(|e| AideMemoError::Serialize {
...
pub fn format_search_results(
    ...
    match format {
        Format::Json => {
            serde_json::to_string_pretty(results).map_err(|e| AideMemoError::Serialize {
...
pub fn format_lint_report(report: &LintReport, format: Format) -> Result<String> {
    match format {
        Format::Json => {
            serde_json::to_string_pretty(report).map_err(|e| AideMemoError::Serialize {
...
pub fn format_stats(stats: &StoreStats, format: Format) -> Result<String> {
    if let Format::Json = format {
        return serde_json::to_string_pretty(stats).map_err(|e| AideMemoError::Serialize {
