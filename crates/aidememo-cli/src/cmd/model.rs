//! `aidememo model` — model management commands.
//!
...
pub fn run_model(config: Config, sub: ModelSub) -> Result<String, AideMemoError> {
    match sub {
...
fn model_list(config: &Config) -> Result<String, AideMemoError> {
...
fn model_status(config: &Config, name: &str) -> Result<String, AideMemoError> {
    let download_dir = expand_path(&config.model.download_dir);
    let cache_dir = expand_path(&config.model.cache_dir);
    ...
    if !local_present && !cache_present {
        out.push_str("Hint: run `aidememo model download <name>` to fetch it.");
    }

    Ok(out.trim_end().to_string())
}

fn model_download(config: &Config, name: &str) -> Result<String, AideMemoError> {
    let download_dir = expand_path(&config.model.download_dir);
    let cache_dir = expand_path(&config.model.cache_dir);
    let download_path = download_dir.join(name);
    ...
    let mut last_error: Option<AideMemoError> = None;
    for bin in ["hf", "huggingface-cli"] {
        match run_huggingface_download(bin, name, &download_path, &cache_dir) {
            Ok(output) => {
                ...
                return Err(AideMemoError::ModelDownloadFailed {
                    name: name.to_string(),
                    source: Box::new(err),
                });
            }
            Err(e) if e.kind() == io::ErrorKind::NotFound => {
                last_error = Some(AideMemoError::ModelDownloadFailed {
                    name: name.to_string(),
                    source: Box::new(e),
                });
                continue;
...
fn discover_model_dirs(root: &Path) -> Result<Vec<ModelDirEntry>, AideMemoError> {
    if !root.exists() {
        return Ok(Vec::new());
    }
...
fn dir_stats(path: &Path) -> Result<(usize, u64), AideMemoError> {
    if !path.exists() {
        return Ok((0, 0));
    }
...
fn model_io_error(action: &str, path: &Path, source: io::Error) -> AideMemoError {
    AideMemoError::Internal(format!(
        "failed to {} {}: {}",
        action,
        path.display(),
