//! `aidememo adapt` — train, inspect, and evaluate the search adapter.

use bpaf::*;
use std::path::Path;

use crate::cmd::Command;
use aidememo_core::{AdaptEvalReport, AdaptResult, AdaptStatus, AideMemo, AideMemoError, Config};
...
fn format_train(result: AdaptResult) -> Result<String, AideMemoError> {
    Ok(format!(
...
fn format_status(status: AdaptStatus) -> Result<String, AideMemoError> {
    Ok(format!(
...
fn format_eval(report: AdaptEvalReport) -> Result<String, AideMemoError> {
    Ok(format!(
