//! `aidememo init` — initialize a new aidememo.
//!
...
use aidememo_core::{AideMemo, AideMemoError, Config, IngestStats};
...
pub fn init_command() -> impl Parser<Command> {
    ...
    let agent = long("agent")
        .help(
            "Also install aidememo into an agent target after init. Supported \
             targets follow `aidememo mcp-install --list-targets` and \
             `aidememo skill install --list-targets`; unsupported surfaces are \
             reported as skipped.",
        )
...
pub fn run_init(
    ...
    if let Some(parent) = store_path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|e| AideMemoError::Internal(format!("failed to create store dir: {}", e)))?;
    }
    steps.push(InitStep {
    ...
    // 5. Print summary
    let mut out = format!("AideMemo initialized at {}\n", store_path.display());
    out.push_str(&format!("Wiki root: {}\n\n", wiki_root.display()));

    if let Some(stats) = stats {
        out.push_str(&format_summary(&stats));
        out.push_str("\nQuick start:\n");
        out.push_str("  aidememo entity list\n");
        out.push_str("  aidememo traverse <entity> --depth 2\n");
        out.push_str("  aidememo search <query>\n");
    } else {
        out.push_str("Store created (ingest skipped).\n");
        out.push_str("Run `aidememo ingest <wiki-root>` when ready.\n");
    }
...
fn step_from_result(
    name: &str,
    result: Result<String, AideMemoError>,
    started: Instant,
) -> InitStep {
    match result {
        Ok(detail) => InitStep {
