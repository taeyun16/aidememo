//! `aidememo skill check` — validate Agent Skills (SKILL.md) files.
//!
...
//!   `Bash(...:...)` or a known MCP tool name (`aidememo_search`,
//!   `aidememo_query`, …). Unknown names warn so users catch typos.
...
pub fn skill_command() -> impl Parser<Command> {
    ...
    .to_options()
    .command("install")
    .help("Install the bundled aidememo skill into an agent's skills directory");
...
fn validate_allowed_tool(spec: &str, mcp_tools: &[String]) -> Option<Issue> {
    ...
    // MCP tool name (e.g. `aidememo_search`)
...
// ---------------------------------------------------------------------------
// `aidememo skill install` — write the bundled skill into an agent's
// skills directory.
// ---------------------------------------------------------------------------
...
const AIDEMEMO_AGENTS_BEGIN: &str = "<!-- BEGIN aidememo-skill -->";
const AIDEMEMO_AGENTS_END: &str = "<!-- END aidememo-skill -->";
...
fn install_into_agents_md(
    ...
    let block = format!(
        "{AIDEMEMO_AGENTS_BEGIN}\n\n# aidememo — AideMemo (auto-installed)\n\
         <!-- to update, run: aidememo skill install --target {target} --force -->\n\n\
         {}\n\
         {AIDEMEMO_AGENTS_END}\n",
        BUNDLED_SKILL_MD
    );
    ...
    let merged = if already {
        ...
        if let Some((before, rest)) = existing.split_once(AIDEMEMO_AGENTS_BEGIN) {
            prefix = before.to_string();
            if let Some((_, after)) = rest.split_once(AIDEMEMO_AGENTS_END) {
...
fn run_install(
    ...
    std::fs::create_dir_all(&resolved_dest)
        .map_err(|e| AideMemoError::Internal(format!("create {}: {e}", resolved_dest.display())))?;

    let skill_path = resolved_dest.join("SKILL.md");
    std::fs::write(&skill_path, BUNDLED_SKILL_MD)
        .map_err(|e| AideMemoError::Internal(format!("write {}: {e}", skill_path.display())))?;

    let ref_path = resolved_dest.join("REFERENCE.md");
    std::fs::write(&ref_path, BUNDLED_REFERENCE_MD)
        .map_err(|e| AideMemoError::Internal(format!("write {}: {e}", ref_path.display())))?;
...
pub fn run_skill(sub: SkillSub, json: bool) -> Result<String, AideMemoError> {
    match sub {
        SkillSub::Install {
            target,
            dest,
        ...
        SkillSub::Check { path } => {
            let files = collect_files(&path).map_err(|e| {
                AideMemoError::FileRead(path.clone(), format!("read skill path: {}", e))
            })?;
            if files.is_empty() {
                return Err(AideMemoError::InvalidInput(format!(
            ...
            for f in &files {
                let report =
                    check_one(f).map_err(|e| AideMemoError::FileRead(f.clone(), e.to_string()))?;
                reports.push(report);
            }
            ...
            if json {
                serde_json::to_string_pretty(&summary).map_err(|e| AideMemoError::Serialize {
...
fn format_human(s: &CheckSummary) -> String {
    ...
    out.push_str(&format!(
        "aidememo skill check — {} file(s)  ({} skill(s) passing, {} doc(s) skipped, {} error(s), {} warning(s))\n\n",
...
mod tests {
    ...
    fn known_mcp_tool_passes() {
        let issues = check_str(
            "---\nname: aidememo\ndescription: A long enough description that makes sense to readers.\nallowed-tools: [aidememo_search, aidememo_query, Bash(aidememo:*)]\n---\n\nbody body body body body body body body body body",
        );
        assert!(
    ...
    fn install_into_agents_md_creates_a_new_file_with_marker() {
        ...
        assert!(out.contains("Installed aidememo skill section"));
        assert!(out.contains("--source-id <namespace>"));
        let body = std::fs::read_to_string(&path).unwrap();
        assert!(body.contains(AIDEMEMO_AGENTS_BEGIN));
        assert!(body.contains(AIDEMEMO_AGENTS_END));
        assert!(body.contains("# aidememo — AideMemo"));
    }
    ...
    fn install_into_agents_md_force_replaces_marker_block_only() {
        ...
        assert!(!body.contains("OLD aidememo content"));
        assert!(body.contains("# aidememo — AideMemo"));
