...
pub enum SyncSub {
    ...
    /// `aidememo sync <DIR>` semantics, kept as the `ingest` subcommand so
    /// existing scripts can be ported with one rename.
    Ingest { wiki_root: PathBuf },
    /// Pull a delta from a remote `aidememo mcp-serve` and apply it to the
...
pub fn build_cli() -> OptionParser<Args> {
    ...
    let project = long("project")
        .short('P')
        .help("Use a registered project (see `aidememo project list`)")
    ...
    let new_cmd = construct!(SessionSub::New { topic })
        ...
        .help(
            "Create a tracked session entity for TOPIC and print \
             `export AIDEMEMO_SESSION_ID=...`. While that env var is set, \
             every `aidememo fact add` auto-attaches the session entity. \
    ...
    let as_of = long("as-of")
        .help(
            "Restrict results to facts current at this date (YYYY-MM-DD \
             or RFC3339). See `aidememo fact list --as-of` for details.",
        )
        .argument::<String>("DATE")
        .optional();
    let limit = long("limit")
    ...
    let current_only = long("current-only")
        .help(
            "Skip facts with `superseded_at` set. Use after \
             `aidememo consolidate --gac` (or any supersede pass) to \
...
fn sync_command() -> impl Parser<Command> {
    // aidememo sync ingest <DIR> — original `aidememo sync` behavior, now namespaced.
    let wiki_root = positional::<PathBuf>("WIKI_ROOT");
    let ingest = construct!(SyncSub::Ingest { wiki_root })
        .to_options()
        .command("ingest")
        .help("Incremental markdown ingest (original `aidememo sync <DIR>` behavior)");

    // aidememo sync pull <URL> [--token T] [--limit N] — Phase 2 pull-only delta.
    let url = positional::<String>("URL")
        .help("Base URL of the upstream `aidememo mcp-serve` (e.g. http://team-host:3000)");
    let token = long("token")
        .help(
            "Bearer token for the upstream. Resolution order: --token > \
             --token-file > AIDEMEMO_MCP_AUTH_TOKEN env > stored auth.json \
             (from `aidememo auth login`). Pass empty string to force no header.",
        )
    ...
    .command("pull")
    .help(
        "Pull a delta from a remote `aidememo mcp-serve` (auto-paginated; --watch for periodic polling)",
    );

    // aidememo sync status [URL] [--json]
    let url = positional::<String>("URL")
    ...
    construct!([ingest, pull, status])
        ...
        .help(
            "Sync the local store. Subcommands: \
             ...
             `pull <URL>` (pull delta from a remote aidememo mcp-serve).",
        )
}
