//! `aidememo edit fact <ID>` — incremental fact content edits.
//!
//! ```text
//! Operations:
...
use aidememo_core::{AideMemo, AideMemoError, Config, FactId, FactUpdate};
use bpaf::*;
use std::path::Path;
...
/// `aidememo fact edit` daemon path. aidememo_fact_edit MCP tool takes the same
...
fn run_fact_edit_via_daemon(
    ...
    let body = serde_json::json!({
        "jsonrpc": "2.0", "id": 1,
        "method": "tools/call",
        "params": {"name": "aidememo_fact_edit", "arguments": args}
    });
    crate::daemon_tool_call(&url, body, "aidememo_fact_edit")
}
