//! `aidememo completions <shell>` — emit shell completion scripts.
//!
//! bpaf ships its own runtime completion handler under the
//! `autocomplete` feature, which we enable in `Cargo.toml`. The
//! handler is wired up automatically: invoking `aidememo --bpaf-complete-
...
//! `aidememo --bpaf-complete-style-bash`) because it's three tiny
    ...
    construct!(CompletionsSub { shell })
        ...
        .help(
            "Emit a shell completion script. Pipe into your shell's \
             ...
             eval \"$(aidememo completions bash)\"\n  \
             aidememo completions zsh > ~/.zsh/completions/_aidememo",
        )
}

pub fn run_completions(sub: CompletionsSub) -> Result<String, AideMemoError> {
    match sub.shell.to_lowercase().as_str() {
        "bash" => Ok(BASH_TEMPLATE.replace("{name}", "aidememo")),
        "zsh" => Ok(ZSH_TEMPLATE.replace("{name}", "aidememo")),
        "fish" => Ok(FISH_TEMPLATE.replace("{name}", "aidememo")),
        "elvish" => Ok(ELVISH_TEMPLATE.replace("{name}", "aidememo")),
...
mod tests {
    ...
    fn emits_bash_template_with_binary_name() {
        ...
        assert!(out.contains("complete -o nosort -F _bpaf_dynamic_completion aidememo"));
    }
    ...
    fn emits_zsh_template() {
        ...
        .unwrap();
        assert!(out.starts_with("#compdef aidememo"));
    }
    ...
    fn emits_fish_template() {
        ...
        .unwrap();
        assert!(out.contains("complete --no-files --command aidememo"));
    }
