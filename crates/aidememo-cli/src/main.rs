...
use aidememo_core::{
    AideMemo, AideMemoError, Config, EntityInput, EntityRecord, EntitySort, EntitySummary,
    EntityType, ExportScope, FactInput, FactListOpts, FactRecord, FactType, LintReport, ListOpts,
    QueryOpts, SearchOpts, TraverseDirection, TraverseOpts, WorkflowStartOpts,
...
fn handle_session(
    ...
    match sub {
        ...
        cmd::SessionSub::Canvas {
            output,
            limit,
            include_superseded,
            session,
        } => with_wiki(store_path, config, |wiki| {
            let session = resolve_session_entity(&wiki, session.as_deref())?;
            let facts = wiki.fact_list(FactListOpts {
                entity_id: Some(session.id),
                limit: Some(limit.unwrap_or(80)),
                current_only: !include_superseded,
                ..Default::default()
            })?;
            let artifact = render_session_canvas(&wiki, &session, &facts)?;
            write_artifact_or_stdout(
                output.as_deref(),
                artifact,
                json,
                serde_json::json!({
...
fn handle_profile(
    ...
    match sub {
        cmd::ProfileSub::Export {
            ...
            let artifact =
                render_project_profile(&wiki, limit.unwrap_or(80), source_id, include_sessions)?;
            write_artifact_or_stdout(
                output.as_deref(),
                artifact.body,
                json,
...
struct ProjectProfileArtifact {
    body: String,
    fact_count: usize,
    entity_count: usize,
}

fn write_artifact_or_stdout(
    output: Option<&Path>,
    body: String,
    json: bool,
