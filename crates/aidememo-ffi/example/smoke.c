/* End-to-end C smoke test for aidememo-ffi.
 *
 * Build (from repo root):
 *   cargo build -p aidememo-ffi
 *   cc crates/aidememo-ffi/example/smoke.c \
 *      -I crates/aidememo-ffi/include \
 *      -L target/debug -laidememo_ffi \
 ...
 * cdylib. The test prefers the staticlib (libaidememo_ffi.a) which links cleanly.
 */
...
int main(void) {
    ...
    CHECK(g != NULL, "aidememo_open");

    char* version = aidememo_version();
    printf("aidememo-ffi version: %s\n", version);
    aidememo_free_string(version);

    /* Entities. */
    char* r = aidememo_entity_add(g, "Redis", "technology",
                            "[\"cache\",\"infra\"]", "[\"redis-server\"]", NULL);
    CHECK(!has_error(r), "entity_add Redis");
    aidememo_free_string(r);

    r = aidememo_entity_add(g, "Postgres", "technology", NULL, NULL, NULL);
    CHECK(!has_error(r), "entity_add Postgres");
    aidememo_free_string(r);

    r = aidememo_resolve_entity(g, "redis-server");
    CHECK(contains(r, "\"id\""), "resolve_entity alias");
    aidememo_free_string(r);

    r = aidememo_entity_get(g, "Redis");
    CHECK(contains(r, "\"name\":\"Redis\""), "entity_get name");
    CHECK(contains(r, "cache"), "entity_get tags");
    aidememo_free_string(r);

    r = aidememo_entity_list(g, 10, NULL);
    CHECK(contains(r, "Redis") && contains(r, "Postgres"), "entity_list");
    aidememo_free_string(r);

    /* Resolve Redis ID for fact_add. */
    r = aidememo_resolve_entity(g, "Redis");
    ...
    char* fact = aidememo_fact_add(g, "Redis Sentinel provides high availability",
                             ids_json, "decision", "[\"ha\"]", NULL, 0.9f);
    CHECK(contains(fact, "\"id\""), "fact_add");
    aidememo_free_string(fact);

    r = aidememo_fact_list(g, "Redis", NULL, 10, false);
    CHECK(contains(r, "Redis Sentinel"), "fact_list");
    aidememo_free_string(r);

    /* Batch insert via aidememo_fact_add_many — JSON array of items. */
    ...
    snprintf(
        ...
        redis_id, redis_id);
    char* batch = aidememo_fact_add_many(g, items_json);
    ...
    aidememo_free_string(batch);

    /* Relations. */
    r = aidememo_relation_add(g, "Redis", "Postgres", "alternative_to");
    CHECK(contains(r, "\"ok\":true"), "relation_add");
    aidememo_free_string(r);

    r = aidememo_relations_get(g, "Redis", "forward");
    CHECK(contains(r, "alternative_to"), "relations_get forward");
    aidememo_free_string(r);

    /* Search. */
    r = aidememo_search(g, "high availability", 5, false);
    CHECK(!has_error(r), "search");
    aidememo_free_string(r);

    /* Traverse, path_find. */
    r = aidememo_traverse(g, "Redis", 1, "both");
    CHECK(contains(r, "entities"), "traverse");
    aidememo_free_string(r);

    r = aidememo_path_find(g, "Redis", "Postgres");
    ...
    r = aidememo_query(g, "Redis", 3, 1, 3, false, "hybrid");
    ...
    char* new_fact = aidememo_fact_add(g, "Redis HA via Sentinel + Cluster",
     ...
     * return one fewer. */
    r = aidememo_fact_list(g, "Redis", NULL, 100, false);
    int all_count = 0;
    {
        const char* it = r;
    ...
    aidememo_free_string(r);
    r = aidememo_fact_list(g, "Redis", NULL, 100, true);
    ...
    printf("OK: aidememo-ffi smoke test passed\n");
    return 0;
}
