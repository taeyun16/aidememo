defmodule Mix.Tasks.Compile.Cargo do
  @moduledoc false
  use Mix.Task.Compiler

  @impl true
  def run(_args) do
    crate_dir = Path.expand(__DIR__)
    workspace = Path.expand("../..", __DIR__)
    profile_args = if Mix.env() == :prod, do: ["--release"], else: []
    target_dir = if Mix.env() == :prod, do: "release", else: "debug"

    {output, status} =
      System.cmd("cargo", ["build", "-p", "aidememo-nif"] ++ profile_args,
        cd: workspace,
        stderr_to_stdout: true
      )

    if status != 0 do
      Mix.raise("cargo build -p aidememo-nif failed:\n#{output}")
    end

    priv = Path.join(crate_dir, "priv")
    File.mkdir_p!(priv)
    build_priv = Path.join(Mix.Project.app_path(), "priv")
    File.mkdir_p!(build_priv)

    src_ext = if :os.type() == {:unix, :darwin}, do: "dylib", else: "so"
    src = Path.join([workspace, "target", target_dir, "libaidememo_nif.#{src_ext}"])
    # Erlang's :erlang.load_nif expects .so on every Unix (incl. macOS).
    source_dst = Path.join(priv, "libaidememo_nif.so")
    build_dst = Path.join(build_priv, "libaidememo_nif.so")

    case File.cp(src, source_dst) do
      :ok ->
        case File.cp(src, build_dst) do
          :ok ->
            {:ok, []}

          {:error, reason} ->
            Mix.raise("failed to copy #{src} → #{build_dst}: #{inspect(reason)}")
        end

      {:error, reason} ->
        Mix.raise("failed to copy #{src} → #{source_dst}: #{inspect(reason)}")
    end
  end
end

defmodule AideMemoNif.MixProject do
  use Mix.Project

  def project do
    [
      app: :aidememo_nif,
      version: "0.1.0",
      elixir: "~> 1.15",
      description: "Elixir bindings for AideMemo (aidememo) — local knowledge graph for LLM agents",
      package: package(),
      deps: deps(),
      compilers: [:cargo | Mix.compilers()]
    ]
  end

  def application, do: [extra_applications: [:logger]]

  defp deps, do: [{:jason, "~> 1.4"}]

  defp package do
    [
      licenses: ["MIT", "Apache-2.0"],
      links: %{"GitHub" => "https://github.com/taeyun16/aidememo"},
      files: ~w(lib mix.exs priv README* LICENSE*)
    ]
  end
end
