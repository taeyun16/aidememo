...
//! over RRF alone on real corpora. aidememo keeps the rerank step opt-in
...
/// configs sigmoid them); aidememo only uses the relative order, so
...
mod tei {
    use super::{AideMemoError, Config, Reranker, Result};
...
///
/// Requires the `fastembed` cargo feature on `aidememo-core`. Default
...
mod fastembed_rerank {
    ...
    impl FastembedReranker {
        pub fn from_config(config: &Config) -> Result<Self> {
            ...
            // Honour the user's aidememo cache_dir (same convention as
            ...
            let inner = TextRerank::try_new(opts).map_err(|e| AideMemoError::ModelLoadFailed {
    ...
    impl Reranker for FastembedReranker {
        ...
        fn rerank(&self, query: &str, texts: &[String]) -> Result<Vec<f32>> {
            ...
            let results = guard
                ...
                .map_err(|e| AideMemoError::Internal(format!("fastembed rerank: {e}")))?;
            // `TextRerank::rerank` returns results sorted by score
            // descending, NOT in input order. aidememo's Reranker contract
...
mod tests {
    ...
    /// dependency; what we're testing is aidememo's *plumbing*, not the
    /// upstream model.
