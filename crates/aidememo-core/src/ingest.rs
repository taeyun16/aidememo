    ...
    for file_path in md_files {
        match process_file(&file_path, &wiki_root) {
            Ok(parsed) => {
                ...
                // searchable. Skipping Unknown was the aidememo-agent-test
...
fn process_file(file_path: &Path, wiki_root: &Path) -> Result<ParsedFile, AideMemoError> {
    let content = std::fs::read_to_string(file_path)
        .map_err(|e| AideMemoError::FileRead(file_path.to_path_buf(), e.to_string()))?;
