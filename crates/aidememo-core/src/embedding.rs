...
//! "make the embedding provider configurable". aidememo's default stays offline
...
//! flipping `model.provider` in `~/.aidememo/config.toml`.
//!
//! Identity contract:
...
/// The set of providers aidememo knows how to construct from `Config`. Useful
    ...
    impl Model2VecProvider {
        pub fn load(config: &Config) -> Result<Self> {
            ...
            let inner = if local_candidate.exists() {
                ...
                return Err(AideMemoError::ModelNotFound {
...
mod tei {
    ...
    fn probe_dimension(embed_endpoint: &str, api_key: Option<&str>) -> Result<usize> {
        ...
        vectors
            ...
            .next()
            .map(|v| v.len())
            .ok_or_else(|| AideMemoError::Internal("tei /embed returned an empty array".into()))
    }
    ...
    impl EmbeddingProvider for TeiProvider {
        ...
        fn embed_batch(&self, texts: &[String]) -> Result<Vec<Vec<f32>>> {
            ...
            // (default 32) — anything larger gets HTTP 413. aidememo's call sites
...
/// `bge-small-en-v1.5`, Mastra's stack) use, while keeping aidememo's
/// default Model2Vec path untouched. Opt in by:
///
///   1. building aidememo with `--features fastembed`
///   2. setting `model.provider = "fastembed"` in `~/.aidememo/config.toml`
...
mod fastembed_provider {
    ...
    impl FastembedProvider {
        pub fn from_config(config: &Config) -> Result<Self> {
            ...
            let inner =
                TextEmbedding::try_new(opts).map_err(|e| AideMemoError::ModelLoadFailed {
    ...
    impl EmbeddingProvider for FastembedProvider {
        ...
        fn embed(&self, text: &str) -> Result<Vec<f32>> {
            ...
            let mut out = guard
                ...
                .map_err(|e| AideMemoError::Internal(format!("fastembed embed: {e}")))?;
            out.pop()
