...
impl<'a, B: StoreBackend + ?Sized> LintEngine<'a, B> {
    ...
    pub fn lint(&self) -> Result<LintReport> {
        ...
        // anyone running with `RUST_LOG=aidememo_core=debug`, plus the
        // legacy AIDEMEMO_LINT_PROFILE eprintln for self-contained dumps.
        let profile = std::env::var("AIDEMEMO_LINT_PROFILE").is_ok();
    ...
    fn check_stale(&self, entities: &[EntitySummary]) -> Result<Vec<LintIssue>> {
        ...
        for entity in entities {
            let record = match self.store.entity_get_by_id(entity.id) {
                Ok(r) => r,
                Err(AideMemoError::EntityIdNotFound(_)) => {
...
fn check_orphans(entities: &[EntitySummary], relations: &[RelationRecord]) -> Vec<LintIssue> {
    ...
    // every entity auto-created by `aidememo_fact_add`, which is the dominant
...
fn check_conflicts(entities: &[EntitySummary], facts: &[FactRecord]) -> Vec<LintIssue> {
    ...
    for ((eid, ft), group) in groups {
        ...
        issues.push(LintIssue {
            ...
            message: format!(
                "Entity '{}' has {} current {:?} facts that may conflict — supersede the stale one with `aidememo fact supersede <old> <new>`. Examples: {}{}",
                entity.name,
                group.len(),
                ft,
                preview.join("; "),
...
mod tests {
    ...
    fn conflict_clears_after_supersede() {
        ...
        // higher-level fact_supersede helper lives on AideMemo,
    ...
    fn orphan_check_does_not_flag_entities_that_have_facts() {
        ...
        // by `aidememo_fact_add` had relations=0 and was tagged orphan, even
