...
use wg_core::{Config, TraverseDirection, WikiGraph};
...
fn relations_for(wiki: &WikiGraph, entity: &str) -> Vec<(String, String, String)> {
...
fn typed_relations_extracted_from_real_markdown() {
    ...
    let entities = wiki.entity_list(wg_core::ListOpts::default()).unwrap();
    for e in &entities {
