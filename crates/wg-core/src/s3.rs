...
impl S3Manifest {
    pub fn open(bucket: impl Into<String>) -> Result<Self> {
        ...
        std::fs::create_dir_all(root.join(SEGMENTS_DIR)).map_err(|source| WgError::StoreOpen {
            path: root.clone(),
...
fn load_manifest(path: &Path) -> Result<Manifest> {
    let content = std::fs::read_to_string(path).map_err(|source| WgError::StoreRead {
        table: "manifest",
...
fn persist_manifest(path: &Path, manifest: &Manifest) -> Result<()> {
    ...
    for entry in manifest {
        let line = serde_json::to_string(entry).map_err(|source| WgError::Serialize {
    ...
    std::fs::write(path, output).map_err(|source| WgError::StoreWrite {
        table: "manifest",
        key: MANIFEST_FILE.to_string(),
        source: Box::new(source),
    })
...
fn current_bucket_root() -> PathBuf {
    std::env::var("WG_STORAGE")
        .map(|value| bucket_root(&value))
        .unwrap_or_else(|_| std::env::temp_dir().join("wg-s3").join("default"))
}
...
mod tests {
    ...
    fn flush_writes_segment() {
        let tmp = tempfile::tempdir().unwrap();
        std::env::set_var("WG_STORAGE", tmp.path());
