...
//! agent can review before committing them with `wg_fact_add{,_many}`.
...
//! themselves and feed structured output to `wg_fact_add_many`. This
...
pub struct ExtractCandidate {
    ...
    /// flow into `wg_fact_add`'s `entities` arg verbatim, so missing
...
pub fn extract_candidates(
    ...
    // afterwards `wg_entity_alias_add` if they want fuzzy aliases.
