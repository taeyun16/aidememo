...
pub fn wal_append(segment: WALSegment) -> Result<SegmentId> {
    ...
    write_txn.commit().map_err(|source| WgError::StoreWrite {
        table: "wal_segments",
        key: segment.segment_id.to_string(),
        source: Box::new(source),
    })?;
...
pub fn wal_segments() -> Result<Vec<WALSegment>> {
    ...
    for entry in table.iter().map_err(|source| WgError::StoreRead {
        table: "wal_segments",
        key: "<iter>".to_string(),
        source: Box::new(source),
    })? {
        let (key, value) = entry.map_err(|source| WgError::StoreRead {
            table: "wal_segments",
            key: "<iter>".to_string(),
            source: Box::new(source),
        })?;
...
pub fn wal_compact(segment_ids: Vec<SegmentId>) -> Result<WALSegment> {
        ...
        })?;
    {
        ...
        let bytes = serde_json::to_vec(&compacted).map_err(|source| WgError::Serialize {
...
fn wal_db() -> Result<Database> {
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|source| WgError::StoreOpen {
            path: path.clone(),
            source: Box::new(source),
        })?;
    }
    let db = Database::create(&path).map_err(|source| WgError::StoreOpen {
        path,
        source: Box::new(source),
    })?;
...
fn wal_path() -> PathBuf {
    if let Ok(storage) = std::env::var("WG_STORAGE") {
    ...
    std::env::temp_dir().join("wg-wal.redb")
}
