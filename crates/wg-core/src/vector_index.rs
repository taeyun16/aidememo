...
//!   sidecar is missing, or rebuilt if `WikiGraph::vector_index_rebuild`
...
impl HnswIndex {
    ...
    pub fn save_to(&self, path: &Path) -> Result<()> {
        ...
        let payload = bincode::serialize(&(&header, &self.map))
            .map_err(|e| WgError::Internal(format!("hnsw serialize failed: {e}")))?;
    ...
    pub fn load_from(path: &Path) -> Result<Option<Self>> {
        ...
        let bytes = std::fs::read(path)
            .map_err(|e| WgError::FileRead(path.to_path_buf(), e.to_string()))?;
        let (header, map): (SidecarHeader, HnswMap<FactPoint, FactId>) =
            bincode::deserialize(&bytes).map_err(|e| {
                WgError::Internal(format!("hnsw deserialize {}: {e}", path.display()))
            })?;
