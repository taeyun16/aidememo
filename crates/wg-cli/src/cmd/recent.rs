...
pub fn run_recent(
    ...
    // Daemon discovery — same pattern as `wg search`/`wg query`.
    // The wg_recent MCP tool returns {"facts": [...]} JSON; we print
...
fn run_recent_via_daemon(base_url: &str, limit: usize, last: &str) -> Result<String, WgError> {
    ...
    crate::daemon_tool_call(&url, body, "wg_recent")
}
