//! Terminal rendering + input loop for `wg pending review`.
//!
...
pub fn run(initial: AppState) -> Result<AppState, WgError> {
    let mut stdout = io::stdout();
    enable_raw_mode().map_err(|e| WgError::Internal(format!("enable_raw_mode: {e}")))?;
    execute!(stdout, EnterAlternateScreen)
        .map_err(|e| WgError::Internal(format!("enter alt screen: {e}")))?;
    let backend = CrosstermBackend::new(stdout);
    let mut terminal =
        Terminal::new(backend).map_err(|e| WgError::Internal(format!("new terminal: {e}")))?;
...
fn run_loop<B: ratatui::backend::Backend>(
    ...
    mut state: AppState,
) -> Result<AppState, WgError> {
    loop {
        terminal
            .draw(|frame| render(frame, &state))
            .map_err(|e| WgError::Internal(format!("draw: {e}")))?;

        // 100 ms poll keeps the loop responsive enough for keypresses
        // without burning CPU when idle.
        if event::poll(Duration::from_millis(100))
            .map_err(|e| WgError::Internal(format!("poll: {e}")))?
        {
            if let Event::Key(key) =
                event::read().map_err(|e| WgError::Internal(format!("read: {e}")))?
            {
