//! `wg auth` — bearer-token UX helpers for the Phase 2 sync surface.
//!
//! Three pieces:
...
//!     token in `~/.wg/auth.json` keyed by the upstream URL. Mode
//!     0600. Subsequent `wg sync pull <URL>` reads it transparently.
//!   * `logout <URL>` / `list` — manage stored entries.
//!
//! Storage lives at `~/.wg/auth.json`; the `wg-cli` `sync pull`
...
pub fn auth_command() -> impl Parser<Command> {
    ...
    let url = positional::<String>("URL")
        .help("Upstream `wg mcp-serve` base URL (e.g. http://team-host:3000)");
...
pub fn run_auth(sub: AuthSub) -> Result<String, WgError> {
    match sub {
        ...
        AuthSub::Login {
            ...
            store.remotes.insert(
                ...
                StoredAuth {
                    token: resolved,
                    added_at: wg_core::time::current_epoch_ms(),
                },
            );
...
/// Read `~/.wg/auth.json` and return the token associated with `url`,
...
fn auth_file_path() -> Result<PathBuf, WgError> {
    let home = std::env::var("HOME").map_err(|_| {
        WgError::InvalidInput("HOME env var not set — can't resolve ~/.wg/auth.json".into())
    })?;
    let dir = PathBuf::from(home).join(".wg");
    if !dir.exists() {
        std::fs::create_dir_all(&dir)
            .map_err(|e| WgError::Internal(format!("create {}: {e}", dir.display())))?;
    }
...
fn save_auth_file(path: &Path, store: &AuthFile) -> Result<(), WgError> {
    let bytes = serde_json::to_vec_pretty(store).map_err(|e| WgError::Serialize {
    ...
    std::fs::rename(&tmp, path).map_err(|e| WgError::Internal(format!("rename auth.json: {e}")))?;
    Ok(())
}
...
fn resolve_token_input(
    ...
    match (token, token_file) {
        (Some(_), Some(_)) => Err(WgError::InvalidInput(
            "pass --token OR --token-file, not both".into(),
        )),
        (Some(t), None) => {
            let trimmed = t.trim().to_string();
            if trimmed.is_empty() {
                return Err(WgError::InvalidInput("--token is empty".into()));
            }
            Ok(trimmed)
        }
        (None, Some(p)) => crate::cmd::mcp_serve::read_token_file(&p),
        (None, None) => Err(WgError::InvalidInput(
            "wg auth login requires --token or --token-file".into(),
        )),
    }
}

fn generate_token_hex(byte_len: usize) -> Result<String, WgError> {
    use std::io::Read;
    let mut buf = vec![0u8; byte_len];
    let mut f = std::fs::File::open("/dev/urandom")
        .map_err(|e| WgError::Internal(format!("open /dev/urandom: {e}")))?;
    f.read_exact(&mut buf)
        .map_err(|e| WgError::Internal(format!("read /dev/urandom: {e}")))?;
