...
use wg_core::{Config, ProjectConfig, WgError};
...
pub fn run_project(mut config: Config, sub: ProjectSub) -> Result<String, WgError> {
    match sub {
        ProjectSub::List => {
            if config.projects.is_empty() {
                return Ok(format!(
                    "No projects registered.\n\nDefault store: {}\nRegister with `wg project create <name> --path <PATH>`.",
        ...
        ProjectSub::Show { name } => {
            let target = match name {
                ...
                None => config
                    .default_project
                    .clone()
                    .ok_or_else(|| WgError::InvalidInput("no default project set".to_string()))?,
            };
            let p = config.projects.get(&target).ok_or_else(|| {
                WgError::InvalidInput(format!("project '{target}' not registered"))
            })?;
            Ok(format!(
                "Project: {target}\n  path: {}\n  resolved: {}",
                p.path,
        ...
        ProjectSub::Create { name, path } => {
            if config.projects.contains_key(&name) {
                return Err(WgError::InvalidInput(format!(
                    "project '{name}' already exists; use `wg project remove {name}` first"
                )));
            }
        ...
        ProjectSub::Use { name } => {
            if !config.projects.contains_key(&name) {
                return Err(WgError::InvalidInput(format!(
                    "project '{name}' not registered; use `wg project create` first"
                )));
            }
            config.default_project = Some(name.clone());
            config.save()?;
