...
- **Phase 2 PostgreSQL production backend completion and artifact conformance status** — PostgreSQL canonical backend now meets all Phase 2 exit gates: concurrent claim/return, restore, replica rebuild, tenant-isolation, and index-rebuild suites pass against both SQLite and PostgreSQL. On-premises S3-compatible (MinIO) artifact conformance is validated via `scripts/artifact-s3-minio-conformance.sh`. Multi-replica artifact support is explicitly gated on Issue #94 (PostgreSQL-backed artifact catalog); the server enforces `--artifact-backend disabled` for PostgreSQL deployments because the current artifact catalog is node-local SQLite. Managed R2/AWS artifact conformance is deliberately not recorded until the catalog is portable to avoid misleading single-node test results. See `docs/ARTIFACT_CONFORMANCE.md` for complete status and honest Phase 2 closeout. **Phase 2 is complete** for the PostgreSQL backend with documented and enforced artifact limitations.
