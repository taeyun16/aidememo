...
> 서버 없이, 설정 없이, `aidememo init && aidememo search` 두 줄이면 시작.
>
...
| 2 | `aidememo init` zero-config 시작 추가 | 첫 경험까지 2줄 (aidememo init → aidememo search) | §3.2 |
| 3 | `aidememo mcp-serve` MCP 서버 모드 추가 | SKILL.md 배포 없이 MCP URL만으로 사용 | §3.2, §1.1 |
| 4 | `aidememo watch` 자동 sync 추가 | 수동 re-ingest 제거 | §3.2 |
| 5 | fact 자동 추출 규칙 구체화 | 수동 `aidememo fact add` 의존 최소화 | §3.2 |
| 6 | 접근 경로를 3-tier로 재정의 | CLI(기본) → MCP(편의) → 바인딩(최적화) | §1.3 |
| 7 | 바인딩을 Phase 5로 재배치 | Phase 2까지만으로 모든 에이전트에서 사용 가능한 제품 | §8 |
| 8 | SKILL.md를 독립 섹션으로 강화 | aidememo의 가장 중요한 에이전트 인터페이스 | §6 |
...
| 7 | MVP에 ingest가 없음 | Phase 1에 `aidememo ingest` + wikilink 파싱 추가 | §3.2, §8 |
...
aidememo-implementation-plan   v0.2.0   Layer 0: Local Index Engine ← 이 문서

관계:
  aidememo는 llm-wiki SKILL.md의 자연스러운 확장.
  어떤 에이전트든 셸에서 aidememo를 호출할 수 있으면 동작.
  개인/소규모 팀 → aidememo (로컬, 서버 없음)
  조직 규모 → Org-Wiki (서버, PG, MCP)
  aidememo의 데이터는 Org-Wiki로 import 가능 (export/import JSONL).
```
...
├── crates/
│   ├── aidememo-core/                # 핵심 라이브러리 (순수 Rust, FFI 무관)
│   │   ├── src/
│   │   │   ├── lib.rs          # AideMemo 공개 API
│   │   │   ├── store.rs        # redb 스토리지
...
│   │   │   ├── config.rs       # ~/.aidememo/config.toml
│   │   │   └── error.rs        # AideMemoError 구조적 에러
│   │   └── Cargo.toml
│   │
│   ├── aidememo-cli/                 # CLI 바이너리 (1차 인터페이스)
│   │   ├── src/
│   │   │   ├── main.rs
│   │   │   ├── cmd/            # bpaf 서브커맨드
│   │   │   │   ├── mod.rs
│   │   │   │   ├── init.rs     # aidememo init (zero-config 시작)
│   │   │   │   ├── ingest.rs   # aidememo ingest, aidememo sync
│   │   │   │   ├── watch.rs    # aidememo watch (자동 re-ingest)
│   │   │   │   ├── traverse.rs
│   │   │   │   ├── search.rs
│   │   │   │   ├── entity.rs
│   │   │   │   ├── fact.rs
│   │   │   │   ├── lint.rs
│   │   │   │   ├── mcp_serve.rs # aidememo mcp-serve (MCP 서버 모드)
│   │   │   │   ├── adapt.rs
│   │   │   │   └── migrate.rs
│   │   │   └── output.rs
│   │   └── Cargo.toml
│   │
│   ├── aidememo-skill/               # SKILL.md + 에이전트별 설정 가이드
│   │   ├── SKILL.md            # 범용 (모든 에이전트)
...
│   │   └── setup-codex.md
│   │
│   ├── aidememo-python/              # PyO3 바인딩 (성능 최적화 경로)
│   │   └── ...
│   ├── aidememo-napi/                # napi-rs 바인딩
│   │   └── ...
│   ├── aidememo-nif/                 # Rustler NIF
│   │   └── ...
│   └── aidememo-ffi/                 # C-ABI
│       └── ...
│
...
1. 에이전트 무관 (Agent-Agnostic)
   aidememo는 CLI 도구이다. 어떤 에이전트든 셸에서 호출할 수 있으면 동작한다.
   Claude Code, Cursor, Codex, Hermes, Copilot, 기타 MCP 클라이언트.
   특정 에이전트에 종속되지 않는다.

2. 설치 = 바이너리 하나
   brew install aidememo 또는 cargo install aidememo-cli.
   Python, Node.js, Docker, 데이터베이스 서버 불필요.

3. 시작 = 두 줄
   aidememo init ~/wiki
   aidememo search "query"
   
4. llm-wiki의 자연스러운 확장
   기존 llm-wiki SKILL.md에 aidememo 섹션을 추가하면
   에이전트가 grep 대신 aidememo를 호출한다.
   aidememo가 없으면 기존 search_files로 fallback.
...
Tier 0 — CLI (기본, 모든 에이전트):
  aidememo 바이너리 설치 → aidememo init → SKILL.md를 에이전트에 배치
  에이전트가 셸에서 aidememo search, aidememo traverse 호출
...
Tier 2 — 언어 바인딩 (성능 최적화, 특정 런타임):
  pip install aidememo-python    → Hermes bridge 플러그인에서 in-process 호출
  npm install @aspect/aidememo   → Node.js 에이전트에서 in-process 호출
  {:aidememo_nif, ...}           → Elixir/Phoenix Org-Wiki에서 NIF 호출
...
```
Claude Code:
  1. brew install aidememo
  2. aidememo init ~/wiki
  3. cp aidememo-skill/SKILL.md ~/.claude/skills/aidememo/
  → 끝. Claude Code가 SKILL.md를 읽고 aidememo를 자동 호출.

Cursor:
  1. brew install aidememo
  2. aidememo init ~/wiki
  3. cp aidememo-skill/SKILL.md .cursor/skills/aidememo/
  → 끝.
...
Hermes Agent:
  방법 A (CLI, 간단):
    ...
    → 끝. Hermes가 셸에서 aidememo 호출.

  방법 B (PyO3, 고성능):
    1. pip install aidememo-python
    2. bridge 플러그인에서 import aidememo
...
MCP 지원 에이전트 (범용):
  1. brew install aidememo
  2. aidememo init ~/wiki
  3. aidememo mcp-serve --port 3000
...
LLM Wiki (마크다운 파일들)
  │
  │  aidememo가 ingest 시 자동 추출
  ▼
aidememo-core (AideMemo)
...
unsafe_code = "forbid"          # aidememo-core에서 unsafe 절대 금지
...
- FFI 바인딩 크레이트(aidememo-python, aidememo-napi, aidememo-nif, aidememo-ffi)에서만 FFI 경계에 필요한 최소 unsafe 허용
- 모든 public API에 `Result<T, AideMemoError>` 반환
...
pub enum AideMemoError {
    ...
    // 마이그레이션
    MigrationFailed { phase: String, source: Box<AideMemoError> },
...
```bash
# 기본: 다국어 (한국어 포함, 128MB)
aidememo config set model.name "minishlab/potion-multilingual-128M"

# 영어 전용 위키라면 경량 모델로 전환 (8MB)
aidememo config set model.name "minishlab/potion-base-8M"

# 고성능 영어 전용 (32MB, 더 큰 어휘)
aidememo config set model.name "minishlab/potion-base-32M"

# 현재 설정 확인
aidememo config get model.name
# → minishlab/potion-multilingual-128M

# 모델 상태 확인
aidememo model status
# → Model: potion-multilingual-128M (128M params, 256 dim, 101 languages)
#   Path: ~/.aidememo/models/potion-multilingual-128M/
#   Size: 128 MB
#   Status: ready
```
...
`~/.aidememo/models/` 에 캐시됨. `aidememo model download` 로 수동 다운로드도 가능.

**config.toml 기본값:**

```toml
# ~/.aidememo/config.toml
[store]
path = "./_meta/wiki.redb"     # 위키 루트 기준 상대 경로

[model]
name = "minishlab/potion-multilingual-128M"   # 기본 모델
cache_dir = "~/.aidememo/models"                    # 모델 캐시 디렉토리
...
```bash
# === ingest (★ MVP 핵심) ===
aidememo ingest <wiki-root>              # 위키 전체 초기 인덱싱
aidememo ingest <wiki-root> --incremental  # 변경분만 재인덱싱
aidememo sync                            # ingest --incremental의 단축
aidememo watch <wiki-root>               # 파일 변경 감지 시 자동 sync (향후)
...
# === 그래프 탐색 ===
aidememo traverse Redis --depth 3
aidememo traverse Redis --depth 2 --relation-type uses,depends_on
aidememo path Redis PostgreSQL

# === 엔티티 관리 ===
aidememo entity add Redis --type technology --tags infra,cache
aidememo entity add Redis --relate "Sentinel:uses" --relate "PostgreSQL:depends_on"
aidememo entity get Redis
aidememo entity rename Redis "Redis Server"     # EntityId 불변, name만 변경
aidememo entity alias Redis --add "redis-cache"  # 별칭 추가
aidememo entity list --sort fact-count --min-facts 2
aidememo entity list --type technology
aidememo entity delete Redis

# === 팩트 관리 ===
aidememo fact add "Sentinel로 HA 구성 결정" --type decision --entities Redis,Sentinel --source "entities/redis.md#ha"
aidememo fact get <fact-id>
aidememo fact list --type decision --min-confidence 0.7 --entities Redis
aidememo fact delete <fact-id>

# === 검색 ===
aidememo search "고가용성"
aidememo search "고가용성" --traverse-from Redis --depth 2
aidememo search "고가용성" --min-confidence 0.5 --limit 20
aidememo search "고가용성" --format json
# 검색 결과에 search_session_id가 포함됨

# === 검색 피드백 (semantic-adapt feature) ===
aidememo feedback click <search_session_id> <fact_id>    # 이 결과를 선택함
aidememo feedback skip <search_session_id> <fact_id>     # 이 결과를 건너뜀

# === 린트 ===
aidememo lint
aidememo lint --format json

# === 어댑터 (semantic-adapt feature) ===
aidememo adapt status
aidememo adapt train
aidememo adapt eval
...
aidememo config set model.name "minishlab/potion-multilingual-128M"
aidememo config set model.name "minishlab/potion-base-8M"
aidememo config set search.default_limit 20
aidememo config list

# === 모델 관리 (semantic feature) ===
aidememo model status                    # 현재 모델 정보
aidememo model download                  # 설정된 모델 수동 다운로드
aidememo model download "minishlab/potion-base-8M"  # 특정 모델 다운로드
aidememo model rebuild-vectors           # 모델 변경 후 벡터 재계산

# === import/export ===
aidememo export > backup.jsonl           # schema_version 포함
aidememo export --scope entities > entities.jsonl
aidememo import < data.jsonl             # schema_version 호환성 체크
...
meta            key: "schema_version"   → u32 (현재: 1)
                ...
                key: "created_by"       → String ("aidememo 0.1.1")
...
3단계: 시맨틱 검색 (semantic feature)
  ...
  → 영어 전용 위키: aidememo config set model.name "minishlab/potion-base-8M" 으로 전환
```
...
```
Phase 1: 피드백 축적 (aidememo 런타임, 자동)
  aidememo search → 결과에 search_session_id 포함
  aidememo feedback click <session_id> <fact_id>    → "이 query에서 이 결과를 선택"
  aidememo feedback skip <session_id> <fact_id>     → "이 query에서 이 결과를 건너뜀"
  → redb search_sessions + search_feedback 테이블에 축적

Phase 2: 어댑터 학습 (aidememo adapt train, 수동/cron)
...
|-------|------|--------|-----------|----------|
| aidememo-cli | Rust 바이너리 | cargo install, homebrew | 터미널, 에이전트 셸 | Tier-1 |
| aidememo-python | PyO3 | pip install aidememo-python | Hermes Agent, Python 스크립트 | Tier-1 |
| aidememo-napi | napi-rs | npm install @aspect/aidememo | Node.js ≥18 | Tier-1 |
| aidememo-napi | napi-rs | (동일 패키지) | Bun ≥1.0 | Best-effort |
| aidememo-napi | napi-rs | (동일 패키지) | Deno (Node 호환 모드) | Experimental |
| aidememo-nif | Rustler | hex (native 빌드) | Elixir/Phoenix (Org-Wiki) | Tier-1 |
| aidememo-ffi | C-ABI | libwg.so/dylib | Clojure/JVM, Swift, Go 등 | Tier-2 |

Bun의 Node-API 호환성은 향상되고 있으나 완전하지 않음.
Deno는 Node 호환 모드에서 동작 가능하나 검증 범위 제한적.
동작하지 않는 경우 aidememo-ffi(C-ABI)로 대체 가능.

### 5.2 aidememo-core와 바인딩의 동시성 모델

aidememo-core 내부는 redb의 concurrent readers + single writer를 활용.
바인딩에서 전체 AideMemo를 하나의 RwLock으로 감싸면 읽기 확장성이 사멸하므로,
구조를 분리한다:
...
```
aidememo-python (PyO3 0.23):
  #[pyclass] struct PyAideMemo(Arc<AideMemo>)
  AideMemo가 Send + Sync이면 PyO3 free-threaded Python 3.13 호환
  별도 RwLock 없이 Arc만으로 충분 (내부 lock이 세밀하게 나뉨)

aidememo-napi (napi-rs):
  #[napi] struct AideMemoStore(Arc<AideMemo>)
  동일 패턴. napi-rs는 Send + Sync만 요구.

aidememo-nif (Rustler):
  ResourceArc<AideMemo>
  Rustler ResourceArc가 thread-safe 참조 제공.
  mutable state는 AideMemo 내부의 RwLock이 처리.
```

### 5.3 Hermes Agent 연동 (aidememo-python)

```python
# bridge 플러그인에서 MCP 왕복 없이 in-process 호출
import aidememo

wiki = aidememo.PyAideMemo("/srv/hermes/members/alice/wiki/_meta/wiki.redb")
...
### 5.4 Node.js/Bun 연동 (aidememo-napi)

```typescript
import { AideMemoStore } from '@aspect/aidememo'  // TypeScript 타입 자동 생성

const wiki = new AideMemoStore('./wiki/_meta/wiki.redb')
...
aidememo의 가장 중요한 에이전트 인터페이스.
이 파일 하나로 Claude Code, Cursor, Codex, Hermes 어디서든 aidememo를 사용.
기존 llm-wiki SKILL.md를 **대체하지 않고 확장**한다.
SKILL.md 전문과 에이전트별 설정 가이드는 aidememo-skill/ 디렉토리에 포함.

aidememo가 없으면 기존 search_files + index.md 방식으로 fallback.
aidememo가 있으면 에이전트가 grep 대신 aidememo search를 호출하여 구조 탐색과 BM25 검색을 활용.
...
```
개인 aidememo 위키  →  Org-Wiki 서버

aidememo export > my_facts.jsonl
...
| aidememo lint | Quality Gate Stage 1 |
...
aidememo가 있어도 Org-Wiki가 필요한 시점:
...
aidememo는 개인/소규모 팀에서 "서버 없이 구조 탐색"을 제공하고,
...
### Phase 1 (3주): store + ingest + aidememo init + traverse

**목표: `aidememo init ~/wiki && aidememo traverse Redis --depth 2` 동작.**

- [ ] workspace 셋업 (Cargo.toml, lints, edition 2024, profile)
- [ ] aidememo-core/types.rs — EntityId(ULID), FactId(ULID), EntityRecord, FactRecord, RelationRecord
- [ ] aidememo-core/error.rs — AideMemoError (구조적 에러, 퍼지 제안, ModelNotFound 등)
- [ ] aidememo-core/config.rs — ~/.aidememo/config.toml 파서 (기본값으로 즉시 동작)
- [ ] aidememo-core/store.rs — redb 열기/닫기, meta 테이블 (schema_version, feature_bitmap)
- [ ] aidememo-core/store.rs — entities + entity_by_name (ULID key + name/alias 보조 인덱스)
- [ ] aidememo-core/store.rs — relations + relations_rev (source_id\0rel_type\0target_id 복합키)
- [ ] aidememo-core/store.rs — facts + fact_by_entity (EntityId 참조)
- [ ] aidememo-core/store.rs — entity/relation/fact CRUD (EntityId canonical, CLI는 name resolve)
- [ ] aidememo-core/store.rs — trust 2축: source_confidence + relevance_score
- [ ] aidememo-core/ingest.rs — 마크다운 파서 (frontmatter YAML, [[wikilink]], heading anchor)
- [ ] aidememo-core/ingest.rs — fact 자동 추출 (decision/convention 헤딩, 마커 패턴)
- [ ] aidememo-core/ingest.rs — aidememo ingest: 전체 스캔 → 엔티티/관계/팩트 자동 추출
- [ ] aidememo-core/ingest.rs — incremental re-ingest (mtime 비교, 변경분만)
- [ ] aidememo-core/ingest.rs — 파일 삭제/이름변경 처리 (EntityId 불변, source_page 갱신)
- [ ] aidememo-core/graph.rs — BFS 탐색 (traverse), 경로 찾기 (path_find), EntityId 기반
- [ ] aidememo-core/fuzzy.rs — 엔티티 퍼지 매칭 (strsim + trigram, entity_by_name 활용)
- [ ] aidememo-core/lib.rs — AideMemo 공개 API
- [ ] aidememo-cli/cmd/init.rs — `aidememo init`: config 생성 + ingest 자동 실행 + 첫 검색 예시 출력
- [ ] aidememo-cli/cmd/ — bpaf 서브커맨드 (init, ingest, sync, entity, fact, traverse, path)
- [ ] aidememo-cli/output.rs — JSON/table/md 출력
...
```
aidememo init ~/wiki
# → "Indexed 42 entities, 156 facts, 89 relations. Try: aidememo search 'Redis'"
aidememo traverse Redis --depth 2
# → 관계 그래프 출력
aidememo entity rename Redis "Redis Server"
# → 관계/팩트에 영향 없음
```
...
SKILL.md를 배치하고 aidememo를 사용할 수 있다.

- [ ] aidememo-core/index.rs — BM25 인메모리 인덱스 빌더 (u32 doc_id ↔ FactId 매핑)
- [ ] aidememo-core/index.rs — dirty flag + lazy rebuild + avgdl 보정 규칙
- [ ] aidememo-core/search.rs — search(), search_with_traverse()
- [ ] aidememo-core/search.rs — combined_score 계산
- [ ] aidememo-core/search.rs — memchr 기반 fallback
- [ ] aidememo-core/lint.rs — 고립 엔티티, 단방향 관계, 중복 후보
- [ ] aidememo-core/migrate.rs — export_jsonl (schema_version), import_jsonl (호환성 체크)
- [ ] aidememo-cli/cmd/ — search, lint, export, import 커맨드
- [ ] aidememo-cli/cmd/watch.rs — `aidememo watch`: 파일 변경 감지 → 자동 re-ingest
- [ ] aidememo-skill/SKILL.md — 범용 SKILL.md 작성 (모든 에이전트용)
- [ ] aidememo-skill/setup-*.md — 에이전트별 설정 가이드 (claude-code, cursor, hermes, codex)
...
```
# Claude Code에서:
cp aidememo-skill/SKILL.md ~/.claude/skills/aidememo/SKILL.md
# → Claude Code가 자동으로 aidememo search 호출

# Cursor에서:
cp aidememo-skill/SKILL.md .cursor/skills/aidememo/SKILL.md
# → 동일하게 동작

# 에이전트 품질: with_aidememo vs without_aidememo에서 +1.0점 이상
```
...
- [ ] aidememo-cli/cmd/mcp_serve.rs — `aidememo mcp-serve`: MCP 서버 모드
- [ ] MCP 도구 정의 (aidememo_search, aidememo_traverse, aidememo_entity_list, aidememo_fact_add, aidememo_lint)
- [ ] MCP 도구 description에 자동 호출 유도 문구 (SKILL.md 불필요하게)
- [ ] aidememo-core/search.rs — Model2Vec 로드/다운로드/캐시 (기본 multilingual-128M)
- [ ] aidememo-core/search.rs — VectorRecord, RRF 합산, 벡터 재계산
- [ ] aidememo-cli/cmd/ — config, model 커맨드
...
- [ ] aidememo feedback click/skip 커맨드
- [ ] DomainAdapter 학습/저장/평가
- [ ] aidememo adapt status/train/eval 커맨드
...
- [ ] aidememo-python — PyO3 (Arc\<AideMemo\>, free-threaded 3.13 호환)
- [ ] aidememo-napi — napi-rs (Node.js Tier-1, Bun best-effort)
- [ ] aidememo-nif — Rustler NIF (Elixir/Phoenix)
- [ ] aidememo-ffi — C-ABI (범용)
- [ ] 각 바인딩 테스트 + 사용 가이드
...
- aidememo-cli (기본 feature): <3 MB (stripped)
- aidememo-cli (전 feature): <5 MB (stripped)
...
aidememo의 실제 효과를 측정하는 5가지 실험.
핵심 질문: **"aidememo가 있으면 에이전트의 지식 작업 품질이 실제로 올라가는가?"**
...
│
└── aidememo_store/
    └── wiki.redb                  # aidememo로 인덱싱된 상태
```

질의 유형을 의도적으로 분류하여 aidememo의 각 구성 요소가 어디서 효과를 내는지 분리 측정:
...
```
질의 유형          설명                              BM25 예상    aidememo 전체 예상
──────────       ──────────────────────            ──────────  ──────────
...
aidememo의 각 구성 요소가 얼마나 기여하는지 분리 측정:

```
A. grep_baseline:  search_files (grep)              ← 현재 llm-wiki 스킬의 기본 방식
B. bm25_only:      aidememo search (BM25만, semantic off)
C. struct_bm25:    aidememo search --traverse-from (구조 탐색 + BM25)
D. full_hybrid:    aidememo search --traverse-from (구조 + BM25 + semantic)
...
semantic/cross_lingual에서 aidememo의 가치가 극명하게 드러남.
...
```
조건 A (without_aidememo):
  에이전트에게 태스크 + 위키 디렉토리 접근 제공
  검색 도구: search_files (grep), read_file
  → 현재 llm-wiki 스킬의 기본 동작

조건 B (with_aidememo):
  에이전트에게 태스크 + aidememo CLI 접근 제공
  검색 도구: aidememo search, aidememo traverse, aidememo entity list, read_file
  → aidememo 통합 llm-wiki 스킬
...
────────────────     ───────   ────────   ──────────   ───────   ──────────   ────
without_aidememo (A)       2.8       2.3        3.1          1.5       3.0          2.54
with_aidememo (B)          4.2       3.8        4.0          3.5       3.6          3.82

→ aidememo 사용 시 평균 +1.28점 (50% 향상)

태스크 유형별 격차:

                     code_ctx   doc_gen   knowledge   debugging   arch_decision
────────────────     ────────   ───────   ─────────   ─────────   ─────────────
without_aidememo           2.8        2.5       2.0         2.5         2.9
with_aidememo              3.9        4.0       4.5         3.5         3.6
격차                  +1.1       +1.5      +2.5        +1.0        +0.7
```

가설: knowledge_retrieval 태스크에서 격차가 가장 큼 (aidememo의 직접적 효과).
코드 작성/디버깅처럼 지식 외 스킬이 중요한 태스크에서는 격차가 상대적으로 작음.


### 10.3 실험 3: 지식 축적 효율 (Knowledge Accumulation)

시간에 따른 위키 성장과 재활용을 측정. aidememo가 "사용할수록 좋아지는가?"를 본다.
...
10일 시뮬레이션 결과:
                     ...
                     without_aidememo      with_aidememo       차이
────────────────     ──────────      ───────       ────
...
핵심: aidememo는 "수동으로는 하지 않았을 지식 구조화"를 자동으로 수행.
typed relation(uses, depends_on, decided_by)이 핵심 차별점.
contradiction 탐지는 aidememo 없이는 아예 불가능.
```
...
Phase 4-5 (바인딩):
  ...
  ✓ 조건 A(without_aidememo) vs B(with_aidememo) 비교
...
│   ├── tasks.jsonl               # 태스크 정의
│   ├── run_agent.py              # 에이전트 실행 (with/without aidememo)
