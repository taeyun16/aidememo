...
공유 메모리와 핸드오프는 연속성의 서로 다른 부분을 해결합니다. 공유 메모리는
연결된 에이전트가 지속 가능한 맥락을 사용할 수 있게 합니다. 핸드오프는 하나의
추적 작업을 `focus`, `done_when`, 같은 세션에 연결되는 결과와 함께 지정한
작업자에게 명시적으로 넘깁니다.

시스템 구성은 [`아키텍처`](ARCHITECTURE.md), 여러 에이전트가 하나의 지속성
있는 저장소를 운영하는 방법은 [`공용 메모리 레이어`](SHARED_MEMORY.md)를
참고하세요. 작업 소유권을 넘기는 방법은
[`추적 작업 핸드오프`](HANDOFF.md), 검증 결과는
[`검증 근거`](EVIDENCE.md), 턴마다 알맞은 도구를 고르는 방법은
[`에이전트 워크플로`](AGENT_WORKFLOWS.md)에 있습니다.
...
6. 작업 주체가 바뀌면 `handoff send`로 다음 작업자가 같은 추적 세션을
   이어받고 근거를 반환하게 합니다.
