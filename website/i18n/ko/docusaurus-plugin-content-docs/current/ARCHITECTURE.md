...
| Server / SSOT 기반 | `crates/aidememo-domain`, `aidememo-service`, `aidememo-store-local`, `aidememo-server`, `aidememo-client`, `aidememo-artifacts` 구현, 제한된 typed session/fact/handoff/artifact HTTP, transactional actor mailbox index, 원시 `custom.*` resource, 격리된 exact-read replica, durable local artifact GC, feature-gated S3/R2 direct-transfer adapter 지원, PostgreSQL과 S3 server wiring, retrieval projection, HTTP MCP gateway routing, offline write는 제안 상태 | [`서버 및 SSOT 아키텍처`](SERVER_SSOT.md) |
