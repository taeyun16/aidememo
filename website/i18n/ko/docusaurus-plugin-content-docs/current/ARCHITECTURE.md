...
| Server / SSOT 기반 | `crates/aidememo-domain`, `aidememo-service`, `aidememo-store-local`, `aidememo-server` 구현, 제한된 typed session/fact/handoff HTTP, transactional actor mailbox index, 원시 `custom.*` resource 지원, PostgreSQL, artifact, search, client/replica API는 제안 상태 | [`서버 및 SSOT 아키텍처`](SERVER_SSOT.md) |
