...
def main() -> None:
    if len(sys.argv) not in (2, 3):
        raise SystemExit("usage: wg_nif_version.py <repo-root> [semver]")
    ...
    mix_path = root / "crates" / "wg-nif" / "mix.exs"
    ...
    if cargo_version != mix_version:
        raise SystemExit(
            "wg-nif version drift: "
            f"Cargo workspace={cargo_version} mix.exs={mix_version}"
        )

    print(f"OK: wg-nif version pinned at {mix_version}")
