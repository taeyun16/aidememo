...
if (!napiDir) {
  console.error('usage: wg_napi_version.mjs <crates/wg-napi> [semver]');
  process.exit(1);
}
...
for (const { dir, pkg } of currentPlatforms) {
  ...
  const expectedDirectory = `crates/wg-napi/npm/${pkg.name}`;
  assert(
...
console.log(
  `OK: wg-napi versions pinned at ${currentRoot.version} across ${currentPlatforms.length} platform packages`,
);
