...
def check_wheel(path: Path, version: str) -> None:
    with zipfile.ZipFile(path) as wheel:
        ...
        if metadata.get("Name") != "wg-python":
        ...
        native_modules = [
            name
            for name in names
            if name.startswith("wg_python/") and name.endswith((".so", ".pyd"))
        ]
...
def check_sdist(path: Path, version: str) -> None:
    expected_root = f"wg_python-{version}/"
    ...
    required = {
        ...
        f"{expected_root}crates/wg-python/src/lib.rs",
...
def main() -> None:
    if len(sys.argv) != 3:
        fail("usage: wg_python_publish_check.py <dist-dir> <version>")

    dist = Path(sys.argv[1]).resolve()
    version = sys.argv[2]
    wheels = sorted(dist.glob("wg_python-*.whl"))
    sdists = sorted(dist.glob("wg_python-*.tar.gz"))

    if len(wheels) != 1:
        fail(f"expected exactly one wg_python wheel in {dist}, got {len(wheels)}")
    if len(sdists) != 1:
        fail(f"expected exactly one wg_python sdist in {dist}, got {len(sdists)}")

    check_wheel(wheels[0], version)
    check_sdist(sdists[0], version)
    print(
        "OK: wg-python publish payload "
