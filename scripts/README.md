...
| `bench-agent-ux.sh` | Small default-feature agent-memory UX regression: Rust check, Hermes tests, shared HTTP/source/actor-default checks, cross-agent packet quality, the three-account pointer-only handoff gate, a real temporary Hermes Kanban lifecycle/external-boundary gate, and fake Codex/Claude external worker-lane success/failure checks. Optional-redb contention scenarios stay in the redb matrix. |
