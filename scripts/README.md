...
| `release-preflight.sh` | One-command release gate with one top-level timed summary. Local profile runs version, changelog release, registry readiness, workflow lint, docs feature gate, docs site e2e, storage backend preflight, binding smoke including agent SDK/Hermes wheel smoke, workflow smoke, and SDK promotion check; full profile adds Rust cargo package readiness, optional binding smokes, and Python-package/npm publish dry-runs. Set `AIDEMEMO_RELEASE_PREFLIGHT_CHANGELOG=0`, `AIDEMEMO_RELEASE_PREFLIGHT_STORAGE_BACKEND=0`, or `AIDEMEMO_RELEASE_PREFLIGHT_CARGO_PACKAGE=0` only for narrow release checks. Set `AIDEMEMO_RELEASE_PREFLIGHT_ACTIONLINT_BIN` to pin or test the actionlint executable. |
| `fresh-checkout-smoke.sh` | Public onboarding smoke: copies the current checkout to a temp directory without build artifacts, verifies `install.sh` syntax, builds `aidememo-cli`, then runs deterministic quickstart `fact add`, `search --bm25-only`, `query --bm25-only`, `workflow start --bm25-only`, and `stats` checks. |
...
| `cargo-package-readiness.sh` | Rust publish-order gate with a timed summary. Runs the release version gate and `cargo package -p aidememo-core`; dependent Rust crates are skipped by default until `aidememo-core` is published to crates.io at the matching version. The CI `cargo-package-readiness` job runs this gate with dependent checks off. Set `AIDEMEMO_CARGO_PACKAGE_CHECK_DEPENDENTS=1` after that first publish to package `aidememo-cli` and the native binding crates. |
| `docs-feature-gate.py` | Public documentation drift gate: verifies CLI/MCP feature inventory, dynamic count claims such as MCP tool and CLI command counts, Docusaurus sidebar/homepage wiring, architecture/workflow/measurement contract tokens, onboarding/community file contracts, Mermaid diagram coverage/static lint, product wording, and the storage positioning that SQLite is default while redb is optional. `--self-test` runs the count-claim drift detector fixture without building the CLI. |
| `docs-site-e2e.py` | Rendered Docusaurus e2e gate: builds the site, verifies sitemap/sidebar/homepage route parity, page H1s, baseUrl-safe links/assets/anchors, and repo-local implementation paths named by the architecture docs. |
