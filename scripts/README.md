...
| `cargo-package-readiness.sh` | Rust publish-order gate with a timed summary. Runs the release version gate and `cargo package -p aidememo-core`; dependent Rust crates are skipped by default until `aidememo-core` is published to crates.io at the matching version. Set `AIDEMEMO_CARGO_PACKAGE_CHECK_DEPENDENTS=1` after that first publish to package `aidememo-cli` and the native binding crates. |
