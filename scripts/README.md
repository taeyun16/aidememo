...
| `aidememo-python-pack-smoke.sh` | Build a `aidememo-python` wheel, install it into a temp venv, run the Python binding smoke, verify wheel metadata matches `aidememo_python.__version__`, and write a timed summary. |
...
| `aidememo-python-publish-dry-run.sh` | Build `aidememo-python` wheel + sdist publish payloads, validate their metadata/file contents without uploading to PyPI, and write a timed summary. |
