...
| `wg-napi-publish-dry-run.sh` | Build, test, and `npm publish --dry-run --access public` the local `wg-napi` package, then verify the publish payload contains JS, types, and the platform `.node` binary. |
