...
| `storage-backend-sqlite-full-surface.sh` | SQLite-only storage smoke: builds the CLI with `--no-default-features --features sqlite`, then verifies init, ingest, entity/fact writes, search/query, graph traversal, sessions, workflow start, archive, export, and import against a single SQLite store. Defaults to the `libsqlite` backend alias; set `AIDEMEMO_SQLITE_FULL_SURFACE_BACKEND=sqlite` to exercise the canonical spelling. |
