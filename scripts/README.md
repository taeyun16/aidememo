...
| `storage-backend-parity.sh` | Storage compatibility gate: verifies redb/SQLite mutation, direct feedback, relation removal, sync compatibility, redb export/import into SQLite, relation preservation, SQLite archive/include-archive, and concurrent SQLite MCP writes. |
...
| `storage-backend-sqlite-mcp-soak.sh` | SQLite-only MCP gate: builds the CLI with `--no-default-features --features sqlite`, runs representative MCP reads/writes (`fact_add_many`, `search`, `query`, `context`, `aggregate`, `workflow_start`, archive/include-archive, graph reads, feedback, extract preview), then verifies concurrent HTTP writes through one SQLite-backed `mcp-serve`. |
| `storage-backend-sdk-bindings-check.sh` | SDK/native binding backend gate: verifies default SQLite builds, explicit SQLite-only builds, `libsqlite` alias opens, redb feature builds, and package-level NIF smoke coverage across Python, Node, Elixir, and C bindings. |
