...
| `skillopt-lite-cycle.sh` | Periodic SkillOpt-lite runner: checks the current memory profile or queued candidates, writes accepted/rejected JSONL records under `target/skillopt-lite`, and applies a passing candidate only with `--apply` / `WG_SKILLOPT_APPLY=1`. |
| `skillopt-lite-check.sh` | Offline SkillOpt-style gate for agent memory skill/profile edits: validates required `wg` memory workflow tokens, runs `wg skill check`, whitespace check, `cargo check -p wg-cli`, the first-run demo, and the SDK promotion gate. Set `WG_SKILLOPT_CANDIDATE=/path/to/SKILL.md` for a candidate and `WG_SKILLOPT_RUN_SCENARIOS=1` for Scenario L/M/N. |
...
| `wg-agent-sdk-pack-smoke.sh` | Build a `wg-agent-sdk` wheel, install it into a temp venv, verify `Memory`, `WgClient`, `WgMemorySDK`, and first-use methods, and write a timed summary. |
| `hermes-wg-pack-smoke.sh` | Build `wg-agent-sdk` and `hermes-wg` wheels, install both into a temp venv, verify SDK re-export, the Hermes plugin entry point, `plugin.yaml`, and bundled `SKILL.md`, and write a timed summary. |
