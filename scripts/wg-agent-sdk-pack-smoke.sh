...
SDK_DIR="$ROOT_DIR/packages/wg-agent-sdk"
EXPECT_VERSION="${WG_AGENT_SDK_EXPECT_VERSION:-}"
BASE="${WG_AGENT_SDK_PACK_SMOKE_BASE:-$(mktemp -d "${TMPDIR:-/tmp}/wg-agent-sdk-pack-smoke.XXXXXX")}"
SUMMARY_TSV="$BASE/wg-agent-sdk-pack-smoke.tsv"
tmp_dir=""
...
run_timed "build wg-agent-sdk wheel" "$venv_dir/bin/python" -m build --wheel --outdir "$dist_dir" "$SDK_DIR"

wheel="$(
    find "$dist_dir" -maxdepth 1 -type f -name 'wg_agent_sdk-*.whl' | sort | head -n 1
)"
if [[ -z "$wheel" ]]; then
    record_fail "wheel artifact" "missing built wg_agent_sdk wheel in $dist_dir"
    exit 1
fi

run_timed "install built wheel" "$venv_dir/bin/python" -m pip --disable-pip-version-check install "$wheel"
run_timed "verify installed wg-agent-sdk payload" "$venv_dir/bin/python" - "$version" <<'PY'
import importlib.metadata
import sys

from wg_agent import Memory, WgClient, WgMemorySDK

expected = sys.argv[1]
metadata_version = importlib.metadata.version("wg-agent-sdk")
if metadata_version != expected:
    raise SystemExit(f"wheel metadata version {metadata_version} != {expected}")

if Memory is not WgMemorySDK:
    raise SystemExit("Memory alias does not point at WgMemorySDK")
...
print(f"installed wg-agent-sdk {metadata_version}; exports Memory, {WgClient.__name__}, {WgMemorySDK.__name__}")
PY
